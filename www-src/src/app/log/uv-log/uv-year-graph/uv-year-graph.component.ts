...
@Component({
  selector: 'dz-uv-year-graph',
  templateUrl: './uv-year-graph.component.html',
  styleUrls: ['./uv-year-graph.component.css']
})
export class UvYearGraphComponent implements OnInit, AfterViewInit {
  ...
  private loadGraph() {
    ...
    const options: Options = {
      chart: {
        ...
        events: {
          load: () => {
            _this.graphService.getData('year', _this.idx, 'uv').subscribe(data => {
      ...
      title: {
        text: 'UV ' + this.translationService.t('Last Year')
      },
      xAxis: {
        type: 'datetime'
      },
