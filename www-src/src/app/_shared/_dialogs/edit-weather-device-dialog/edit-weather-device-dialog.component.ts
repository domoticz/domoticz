...
@Component({
  selector: 'dz-edit-weather-device-dialog',
  templateUrl: './edit-weather-device-dialog.component.html',
  styleUrls: ['./edit-weather-device-dialog.component.css']
})
export class EditWeatherDeviceDialogComponent extends DialogComponent implements OnInit, AfterViewInit {
  ...
  protected getDialogId(): string {
    return 'dialog-editweatherdevice';
  }
  ...
  protected getDialogButtons(): any {
    const dialog_editweatherdevice_buttons = {};

    dialog_editweatherdevice_buttons[this.translationService.t('Update')] = () => {
      let bValid = true;
    ...
    dialog_editweatherdevice_buttons[this.translationService.t('Remove Device')] = () => {
      this.close();
    ...
    dialog_editweatherdevice_buttons[this.translationService.t('Replace')] = () => {
      this.close();
      this.replaceDeviceService.ReplaceDevice(this.item.idx, this.replaceCallbackFn);
    };

    dialog_editweatherdevice_buttons[this.translationService.t('Cancel')] = () => {
      this.close();
    };

    return dialog_editweatherdevice_buttons;
  }
