...
@Component({
  selector: 'dz-edit-distance-device-dialog',
  templateUrl: './edit-distance-device-dialog.component.html',
  styleUrls: ['./edit-distance-device-dialog.component.css']
})
export class EditDistanceDeviceDialogComponent extends DialogComponent implements OnInit, AfterViewInit {
  ...
  protected getDialogId(): string {
    return 'dialog-editdistancedevice';
  }
  ...
  protected getDialogButtons(): any {
    const dialog_editdistancedevice_buttons = {};

    dialog_editdistancedevice_buttons[this.translationService.t('Update')] = () => {
      let bValid = true;
      bValid = bValid && Utils.checkLength(this.item.Name, 2, 100);
      if (bValid) {
        this.close();
        this.deviceService.updateDistanceDevice(this.item.idx, this.item.Name, this.item.Description, this.item.SwitchTypeVal)
          .pipe(
            // Other fields can change, so need to retrieve the device again
    ...
    dialog_editdistancedevice_buttons[this.translationService.t('Remove Device')] = () => {
      this.close();
    ...
    dialog_editdistancedevice_buttons[this.translationService.t('Cancel')] = () => {
      this.close();
    };

    return dialog_editdistancedevice_buttons;
  }
