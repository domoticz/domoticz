...
@Component({
  selector: 'dz-edit-baro-device-dialog',
  templateUrl: './edit-baro-device-dialog.component.html',
  styleUrls: ['./edit-baro-device-dialog.component.css']
})
export class EditBaroDeviceDialogComponent extends DialogComponent implements OnInit, AfterViewInit {
  ...
  protected getDialogButtons(): any {
    const dialog_editbarodevice_buttons = {};

    dialog_editbarodevice_buttons[this.translationService.t('Update')] = () => {
      let bValid = true;
      ...
      if (bValid) {
        ...
        this.deviceService.updateBaroDevice(this.item.idx, this.item.Name, this.item.Description, this.item.AddjValue2).pipe(
    ...
    dialog_editbarodevice_buttons[this.translationService.t('Remove Device')] = () => {
      this.close();
    ...
    dialog_editbarodevice_buttons[this.translationService.t('Replace')] = () => {
      this.close();
      this.replaceDeviceService.ReplaceDevice(this.item.idx, this.replaceCallbackFn);
    };

    dialog_editbarodevice_buttons[this.translationService.t('Cancel')] = () => {
      this.close();
    };

    return dialog_editbarodevice_buttons;
  }
