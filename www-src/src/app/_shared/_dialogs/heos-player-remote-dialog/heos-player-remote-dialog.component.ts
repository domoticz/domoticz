...
@Component({
  selector: 'dz-heos-player-remote-dialog',
  templateUrl: './heos-player-remote-dialog.component.html',
  styleUrls: ['./heos-player-remote-dialog.component.css']
})
export class HeosPlayerRemoteDialogComponent extends DialogComponent implements OnInit, AfterViewInit {

  Name: string;
  devIdx: string;

  viewBox = { x: 50, y: -150, width: 900, height: 1100 };
  ...
  protected getDialogId(): string {
    return 'dialog-heosplayer-remote';
  }
  ...
  public clickHeosplayerRemote(action: string) {
    if (this.devIdx.length > 0) {
      this.lightSwitchesServices.mediaRemote('heosmediacommand', this.devIdx, action).subscribe(() => {
        // Nothing
      }, error => {
