...
void CSQLHelper::PerformThreadedAction(const _tTaskItem &itt)
{
	if (itt._ItemType == TITEM_EXECUTESHELLCOMMAND)
	{
		...
		else
		{
			if (timeout > 0)
			{
				T = std::make_shared<std::thread>([this,pid,timeout,stillRunning,timeoutOccurred] () mutable { ManageExecuteScriptTimeout(pid, timeout, &stillRunning, &timeoutOccurred);});
			}
