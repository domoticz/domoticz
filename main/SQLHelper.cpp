...
bool CSQLHelper::OpenDatabase()
{
	...
	if (rc)
	{
		_log.Log(LOG_ERROR, "Error opening %s SQLite3 database: %s", m_dbase_name.c_str(), sqlite3_errmsg(m_dbase));
	...
	int dbversion = 0;
	if (bNewInstall)
  		_log.Log(LOG_STATUS, "Creating %s SQLite3 database (version: %d)", m_dbase_name.c_str(), DB_VERSION);
	else
	{
		GetPreferencesVar("DB_Version", dbversion);
 		if (dbversion == DB_VERSION)
  			_log.Log(LOG_STATUS, "Opened %s SQLite3 database (version: %d)", m_dbase_name.c_str(), DB_VERSION);
		else if (dbversion < DB_VERSION)
  			_log.Log(LOG_STATUS, "Upgrade %s SQLite3 database from version %d to %d", m_dbase_name.c_str(), dbversion, DB_VERSION);
		else
		{
			//User is using a newer database on a old Domoticz version
			//This is very dangerous and should not be allowed
			_log.Log(LOG_ERROR, "%s SQLite3 database incompatible with this Domoticz version. (Cannot downgrade version from %d to %d!)", m_dbase_name.c_str(), dbversion, DB_VERSION);
			sqlite3_close(m_dbase);
			m_dbase = nullptr;
			return false;
		}
