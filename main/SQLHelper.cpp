...
uint64_t CSQLHelper::UpdateValueInt(const int HardwareID, const char* ID, const unsigned char unit, const unsigned char devType, const unsigned char subType, const unsigned char signallevel, const unsigned char batterylevel, const int nValue, const char* sValue, std::string &devname, const bool bUseOnOffAction)
{
	...
	if (result.size()==0)
	{
		...
		{
			std::vector<std::string> sd = result[0];
			_eHardwareTypes htype = (_eHardwareTypes)atoi(sd[0].c_str());
			if (htype == HTYPE_PythonPlugin)
			{
				// Restart plugin to sync plugins' device list
				std::stringstream ss;
				ss << HardwareID;
				_log.Log(LOG_TRACE, "CSQLHelper::UpdateValueInt: Restarting plugin '%s'", ss.str().c_str());
				m_mainworker.RestartHardware(ss.str());
			}
		}
		_log.Log(LOG_TRACE,"CSQLHelper::UpdateValueInt Adding device to database");
...
void CSQLHelper::DeleteDevices(const std::string &idx)
{
	...
	if (!_idx.empty())
	{
		std::set<std::string> pluginhwids;
		...
		for (itt = _idx.begin(); itt != _idx.end(); ++itt)
		{
			...
			std::vector<std::vector<std::string> > result;
			result = safe_query("SELECT HardwareID FROM DeviceStatus WHERE (ID == '%q')", (*itt).c_str());
			if (result.size() > 0)
			{
				std::vector<std::string> sd = result[0];
				std::string HwID = sd[0];
				result = safe_query("SELECT Type FROM Hardware WHERE (ID=='%q')", HwID.c_str());
				if (result.size() > 0)
				{
					sd = result[0];
					_eHardwareTypes htype = (_eHardwareTypes)atoi(sd[0].c_str());
		...
		for (it = pluginhwids.begin(); it != pluginhwids.end(); ++it)
		{
			// Restart plugin to sync plugins' device list
			_log.Log(LOG_TRACE, "CSQLHelper::DeleteDevices: Restarting plugin");
			m_mainworker.RestartHardware(*it);
		}
	}
	else
		return;
