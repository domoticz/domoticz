...
namespace http {
	namespace server {
		...
		void CWebServer::Cmd_GetCosts(Json::Value &root)
		{
			...
			char szTmp[100];
			std::vector<std::vector<std::string> > result;
			result = m_sql.safe_query("SELECT Type, nValue, sValue FROM DeviceStatus WHERE (ID=='%q')",
				idx.c_str());
			if (result.size() > 0)
			{
				std::vector<std::string> sd = result[0];
				...
				nValue = (unsigned char)atoi(sd[1].c_str());
		...
		void CWebServer::HandleCommand(const std::string &cparam, Json::Value &root)
		{
			...
			else if (cparam == "clearlightlog")
			{
				...
				m_sql.safe_query("DELETE FROM LightingLog WHERE (DeviceRowID=='%q')",
					idx.c_str());
			}
		...
		void CWebServer::Cmd_DeleteCustomIcon(Json::Value &root)
		{
			...
			m_sql.safe_query("DELETE FROM CustomImages WHERE (ID == %d)",
				idx);
		...
		void CWebServer::Cmd_RenameDevice(Json::Value &root)
		{
			...
			m_sql.safe_query("UPDATE DeviceStatus SET Name='%q' WHERE (ID == %d)",
				sname.c_str(), idx);
		}
		...
		void CWebServer::Cmd_SetUnused(Json::Value &root)
		{
			...
			m_sql.safe_query("UPDATE DeviceStatus SET Used=0 WHERE (ID == %d)",
				idx);
		}
		...
		void CWebServer::RType_SetSharedUserDevices(Json::Value &root)
		{
			...
			{
				m_sql.safe_query("INSERT INTO SharedDevices (SharedUserID,DeviceRowID) VALUES ('%q','%q')",
		...
		void CWebServer::RType_SetUsed(Json::Value &root)
		{
			...
			m_sql.safe_query("UPDATE DeviceStatus SET Protected=%d WHERE (ID == '%q')",
