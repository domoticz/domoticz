...
namespace http {
	namespace server {
		...
		void CWebServer::Cmd_AddHardware(Json::Value &root)
		{
			...
			std::string filename = CURLEncode::URLDecode(m_pWebEm->FindValue("filename"));
			...
			m_sql.safe_query(
				"INSERT INTO Hardware (Name, Enabled, Type, Address, Port, SerialPort, Username, Password, FileName, Mode1, Mode2, Mode3, Mode4, Mode5, Mode6, DataTimeout) VALUES ('%q',%d, %d,'%q',%d,'%q','%q','%q','%q',%d,%d,%d,%d,%d,%d,%d)",
				name.c_str(),
				(senabled == "true") ? 1 : 0,
				htype,
				address.c_str(),
			...
			if (result.size() > 0)
			{
				...
				m_mainworker.AddHardwareFromParams(ID, name, (senabled == "true") ? true : false, htype, address, port, sport, username, password, filename, mode1, mode2, mode3, mode4, mode5, mode6, iDataTimeout, true);
			}
		}
		...
		void CWebServer::Cmd_UpdateHardware(Json::Value &root)
		{
			...
			{
				m_sql.safe_query(
					"UPDATE Hardware SET Name='%q', Enabled=%d, Type=%d, Address='%q', Port=%d, SerialPort='%q', Username='%q', Password='%q', FileName='%q', Mode1=%d, Mode2=%d, Mode3=%d, Mode4=%d, Mode5=%d, Mode6=%d, DataTimeout=%d WHERE (ID == '%q')",
					name.c_str(),
					(bEnabled == true) ? 1 : 0,
					htype,
					address.c_str(),
			...
			else
			{
				...
				m_mainworker.AddHardwareFromParams(ID, name, bEnabled, htype, address, port, sport, username, password, filename, mode1, mode2, mode3, mode4, mode5, mode6, iDataTimeout, true);
			}
		}
		...
		void CWebServer::RType_Hardware(Json::Value &root)
		{
			...
			std::vector<std::vector<std::string> > result;
			result = m_sql.safe_query("SELECT ID, Name, Enabled, Type, Address, Port, SerialPort, Username, Password, FileName, Mode1, Mode2, Mode3, Mode4, Mode5, Mode6, DataTimeout FROM Hardware ORDER BY ID ASC");
			if (result.size() > 0)
			{
				std::vector<std::vector<std::string> >::const_iterator itt;
				int ii = 0;
