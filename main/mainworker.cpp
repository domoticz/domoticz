...
bool MainWorker::SwitchScene(const uint64_t idx, std::string switchcmd, const std::string& User)
{
	...
	m_sql.safe_query("UPDATE Scenes SET nValue=%d, LastUpdate='%q', User='%q' WHERE (ID == %" PRIu64 ")",
		nValue,
		szLastUpdate.c_str(),
		User.c_str(),
		idx);
	...
	if (m_sql.m_bEnableEventSystem)
		bEventTrigger = m_eventsystem.UpdateSceneGroup(idx, nValue, szLastUpdate, User.c_str());
