...
void CEventSystem::GetCurrentStates()
{
	...
	std::map<uint64_t, _tHardwareListIntEV> _hardwareNames;
	...
	if (result.size()>0)
	{
		...
		for (itt = result.begin(); itt != result.end(); ++itt)
		{
			...
			std::map<uint64_t, _tHardwareListIntEV>::const_iterator hItt = _hardwareNames.find(HwID);
...
static inline int64_t GetIndexFromDevice(std::string devline)
{
	...
	return (int64_t)atoll(devline.c_str());
}
