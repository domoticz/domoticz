...
void CEventSystem::LoadEvents()
{
	...
	std::string t = "--[[\nThis script is generated by Domoticz\nDo not edit this file.\n]]--\n\n";
#ifdef WIN32
	dzv_Dir = szUserDataFolder + "scripts\\dzVents\\scripts\\db\\";
#else
	dzv_Dir = szUserDataFolder + "scripts/dzVents/scripts/db/";
#endif
	...
	if (result.size()>0)
	{
		...
		for (itt = result.begin(); itt != result.end(); ++itt)
		{
			...
			{
				_log.Log(LOG_STATUS, "EventSystem: Write file: %s",eitem.Name.c_str());
				s = dzv_Dir + eitem.Name.c_str() + ".lua";
				FILE *fOut = fopen(s.c_str(), "wb+");
				if (fOut)
				{
					fwrite(t.c_str(), 1, t.size(), fOut);
