	...
	const std::string &devname)
{
	...
	if (g_bUseEventTrigger && GetEventTrigger(ulDevID, REASON_DEVICE, true))
	{
		...
		item.sValue = osValue;
		item.nValueWording =
			UpdateSingleState(ulDevID, devname, nValue, osValue, devType, subType, switchType, "", 255, batterylevel, options);
		boost::unique_lock<boost::shared_mutex> devicestatesMutexLock(m_devicestatesMutex);
		auto itt = m_devicestates.find(ulDevID);
		if (itt != m_devicestates.end())
		{
		...
		UpdateSingleState(ulDevID, devname, nValue, osValue, devType, subType, switchType, lastUpdate, lastLevel, batterylevel,
				  options);
}
	...
	do
	{
		...
		else if (deviceName.find("Text:") == 0)
		{
			...
			if (parseResult.fAfterSec < (1. / timer_resolution_hz / 2))
				...
				m_sql.AddTaskItem(_tTaskItem::UpdateDevice(parseResult.fAfterSec, std::stoull(variableName), 0,
									   parseResult.sCommand, false, false));
