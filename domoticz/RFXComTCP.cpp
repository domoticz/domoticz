...
RFXComTCP::RFXComTCP(const int ID, const std::string IPAddress, unsigned short usIPPort)
{
	HwdID=ID;
#if defined WIN32
	int ret;
...
void RFXComTCP::WriteToDevice(const char *pdata, const unsigned char length)
{
	if (isConnected())
		write(pdata,length);
}
