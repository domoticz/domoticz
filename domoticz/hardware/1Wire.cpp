...
void C1Wire::DetectSystem()
{
   ...
   if (!m_system && C1WireByKernel::IsAvailable())
   ...
   m_system->GetDevices(devices);
	...
	for (itt=devices.begin(); itt!=devices.end() && !m_stoprequested; ++itt)
	{
      ...
      case _4k_ram_with_counter:
         {
            ReportCounter(device.devid,m_system->GetCounter(device,0));
            break;
         }
...
void C1Wire::ReportTemperature(const std::string& deviceId,float temperature)
{
   ...
   tsen.TEMP.rssi=6;
   tsen.TEMP.id1=(BYTE)deviceIdByteArray[1];
   tsen.TEMP.id2=(BYTE)deviceIdByteArray[0];
...
void C1Wire::ReportLightState(const std::string& deviceId,int unit,bool state)
{
   ...
   tsen.LIGHTING2.id1=(BYTE)deviceIdByteArray[3];
   tsen.LIGHTING2.id2=(BYTE)deviceIdByteArray[2];
   tsen.LIGHTING2.id3=(BYTE)deviceIdByteArray[1];
   tsen.LIGHTING2.id4=(BYTE)deviceIdByteArray[0];
...
void C1Wire::ReportCounter(const std::string& deviceId,unsigned long counter)
{
   unsigned char deviceIdByteArray[DEVICE_ID_SIZE]={0};
   DeviceIdToByteArray(deviceId,deviceIdByteArray);
   ...
   tsen.RFXMETER.id1=(BYTE)deviceIdByteArray[1];
   tsen.RFXMETER.id2=(BYTE)deviceIdByteArray[0];
