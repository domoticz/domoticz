...
OTGWTCP::OTGWTCP(const int ID, const std::string IPAddress, const unsigned short usIPPort)
{
	m_HwdID=ID;
#if defined WIN32
	int ret;
...
bool OTGWTCP::ConnectInternal()
{
	...
	if (nRet == SOCKET_ERROR)
	{
		...
		m_socket=INVALID_SOCKET;
		_log.Log(LOG_ERROR,"OTGW could not connect to: %s:%ld",m_szIPAddress.c_str(),m_usIPPort);
		return false;
	}
	...
	while (!m_stoprequested)
	{
		...
		else
		{
			...
			if (m_stoprequested)
				break;
			if ((bread==0)||(bread<0)) {
				_log.Log(LOG_ERROR,"OTGW TCP/IP connection closed!");
				closesocket(m_socket);
				m_socket=INVALID_SOCKET;
				if (!m_stoprequested)
				{
...
void OTGWTCP::write(const char *data, size_t size)
{
}
