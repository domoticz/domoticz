...
void blebox::widgets::widget::send_rgbw(BleBox &bb, uint32_t color) const {
  ...
  uint8_t level = w * 100 / 255; // TODO: avoid rounding?
...
// TODO: test properly
uint32_t blebox::widgets::widget::adjust_rgbw_brightness(const _tColor &color,
                                                         uint8_t brightness) {
  uint8_t r = color.r;
  uint8_t g = color.g;
  uint8_t b = color.b;
  ...
  double h, s, p;
  RGBtoHSP(r, g, b, &h, &s, &p);
  HSPtoRGB(h, s, (brightness / 255.0), &r, &g, &b);

  uint32_t new_color = uint32_t(r << 24) | uint32_t(g << 16) |
                       uint32_t(b << 8) | uint32_t(brightness);
...
void blebox::widgets::widget::verify_response_value_matches(
  ...
  if (value != jvalue) {
    ...
    os << "failed to set value for " << field()
       << "(expected: " << jvalue.asString() << ", got: " << value.asString()
       << ")";
