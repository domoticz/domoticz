	...
	if (!root["body"]["home"]["rooms"].empty())
	{
		...
		for (auto room : mRoot)
		{
			if (!room["id"].empty())
			{
				...
				{
					SendTempSensor((roomID & 0x00FFFFFF) | 0x03000000, 255,
						       room["therm_measured_temperature"].asFloat(), roomName);
				}
				if (!room["therm_setpoint_temperature"].empty())
				{
					SendSetPointSensor((uint8_t)(((roomID & 0x00FF0000) | 0x02000000) >> 16), (roomID & 0XFF00) >> 8,
							   roomID & 0XFF, room["therm_setpoint_temperature"].asFloat(), roomName);
				}
