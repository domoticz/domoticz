...
{
	values.erase(std::remove_if(values.begin(), values.end(), [id](auto value) { return value->id == id; }), values.end());

	bool found = std::any_of(values.begin(), values.end(), [id](auto value) { return value->id == id; });
	if (!found)
	{
#ifdef OZW_WRITE_LOG
...
#endif
	auto it = std::find_if(groups.begin(), groups.end(), [grp](auto group) { return group->groupid == grp; });
	if (it == groups.end()) {
#ifdef OZW_WRITE_LOG
