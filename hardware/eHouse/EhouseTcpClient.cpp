...
//Can Proceed then submit to TCPClient queue
	...
	ExecQueuedEvents();
#else
	EhouseTcpClientThread[tcp_client_socket_index] = std::shared_ptr<std::thread>(new std::thread(std::bind(&eHouseTCP::EhouseSubmitData, this, tcp_client_socket_index)));
