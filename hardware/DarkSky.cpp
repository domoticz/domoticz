...
void CDarkSky::GetMeterDetails()
{
	...
	*/
	...
	{
		humidity=std::lrint(root["currently"]["humidity"].asFloat()*100.0f);
	}
	...
	if (wind_degrees!=-1)
	{
		...
		int sw=std::lrint(windspeed_ms*10.0f);
		...
		tsen.WIND.gusth=0;
		tsen.WIND.gustl=0;
		int gw=std::lrint(windgust_ms*10.0f);
