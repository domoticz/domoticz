			...
			else
			{
				...
				case P1TYPE_POWERUSAGE:
					...
					if ((l_buffer[8] & 0xFE) == 0x30)
					{
						...
						// Unsigned underflow when temp_usage < previous → result wraps to huge value → correctly rejected.
...
bool P1MeterBase::CheckCRC()
{
	...
	// CRC was computed incrementally during parsing — no need to loop again
