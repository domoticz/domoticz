...
constexpr std::array<uint8_t, 5> V6_PreAmble{ 0x80, 0x00, 0x00, 0x00, 0x11 };

constexpr std::array<uint8_t, 27> V6_GetSessionID{ 0x20, 0x00, 0x00, 0x00, 0x16, 0x02, 0x62, 0x3A, 0xD5, 0xED, 0xA3, 0x01, 0xAE, 0x08,
						   0x2D, 0x46, 0x61, 0x41, 0xA7, 0xF6, 0xDC, 0xAF, 0xD3, 0xE6, 0x00, 0x00, 0x1E };

constexpr std::array<uint8_t, 10> V6_BridgeOn{ 0x31, 0x00, 0x00, 0x00, 0x03, 0x03, 0x00, 0x00, 0x00, 0x01 };
constexpr std::array<uint8_t, 10> V6_RGBWW_On{ 0x31, 0x00, 0x00, 0x08, 0x04, 0x01, 0x00, 0x00, 0x00, l_zone };
constexpr std::array<uint8_t, 10> V6_RGBW_On{ 0x31, 0x00, 0x00, 0x07, 0x03, 0x01, 0x00, 0x00, 0x00, l_zone };

constexpr std::array<uint8_t, 10> V6_BridgeOff{ 0x31, 0x00, 0x00, 0x00, 0x03, 0x04, 0x00, 0x00, 0x00, 0x01 };
constexpr std::array<uint8_t, 10> V6_RGBWW_Off{ 0x31, 0x00, 0x00, 0x08, 0x04, 0x02, 0x00, 0x00, 0x00, l_zone };
constexpr std::array<uint8_t, 10> V6_RGBW_Off{ 0x31, 0x00, 0x00, 0x07, 0x03, 0x02, 0x00, 0x00, 0x00, l_zone };

constexpr std::array<uint8_t, 10> V6_Bridge_SetColor{ 0x31, 0x00, 0x00, 0x00, 0x01, 0xBA, 0xBA, 0xBA, 0xBA, 0x01 };
constexpr std::array<uint8_t, 10> V6_RGBWW_SetColor{ 0x31, 0x00, 0x00, 0x08, 0x01, 0xBA, 0xBA, 0xBA, 0xBA, l_zone };
constexpr std::array<uint8_t, 10> V6_RGBW_SetColor{ 0x31, 0x00, 0x00, 0x07, 0x01, 0xBA, 0xBA, 0xBA, 0xBA, l_zone };

constexpr std::array<uint8_t, 10> V6_RGBWW_SetKelvinLevel{
...
constexpr std::array<uint8_t, 10> V6_Bridge_SetBrightnessLevel{ 0x31, 0x00, 0x00, 0x00, 0x02, 0xBE, 0x00, 0x00, 0x00, 0x01 };
constexpr std::array<uint8_t, 10> V6_RGBWW_SetBrightnessLevel{ 0x31, 0x00, 0x00, 0x08, 0x03, 0xBE, 0x00, 0x00, 0x00, l_zone };
constexpr std::array<uint8_t, 10> V6_RGBW_SetBrightnessLevel{ 0x31, 0x00, 0x00, 0x07, 0x02, 0xBE, 0x00, 0x00, 0x00, l_zone };

constexpr std::array<uint8_t, 10> V6_Bridge_White_On{ 0x31, 0x00, 0x00, 0x00, 0x03, 0x05, 0x00, 0x00, 0x00, 0x01 };
constexpr std::array<uint8_t, 10> V6_RGBWW_White_On{ 0x31, 0x00, 0x00, 0x08, 0x05, 0x64, 0x00, 0x00, 0x00, l_zone };
constexpr std::array<uint8_t, 10> V6_RGBW_White_On{ 0x31, 0x00, 0x00, 0x07, 0x03, 0x05, 0x00, 0x00, 0x00, l_zone };

constexpr std::array<uint8_t, 10> V6_RGBWW_Night_On{ 0x31, 0x00, 0x00, 0x08, 0x04, 0x05, 0x00, 0x00, 0x00, l_zone };
constexpr std::array<uint8_t, 10> V6_RGBW_Night_On{ 0x31, 0x00, 0x00, 0x07, 0x03, 0x06, 0x00, 0x00, 0x00, l_zone };

constexpr std::array<uint8_t, 10> V6_Bridge_Disco_Mode{
	0x31, 0x00, 0x00, 0x00, 0x04, 0x01, 0x00, 0x00, 0x00, 0x01
}; //(6th hex values 0x01 to 0x09 : examples: 01 = Mode1, 02 = Mode2, 03 = Mode3 .. 09 = Mode9)
constexpr std::array<uint8_t, 10> V6_RGBWW_Disco_Mode{
	0x31, 0x00, 0x00, 0x08, 0x06, 0x01, 0x00, 0x00, 0x00, l_zone
}; //(6th hex values 0x01 to 0x09 : examples: 01 = Mode1, 02 = Mode2, 03 = Mode3 .. 09 = Mode9)
constexpr std::array<uint8_t, 10> V6_RGBW_Disco_Mode{
	0x31, 0x00, 0x00, 0x07, 0x04, 0x01, 0x00, 0x00, 0x00, l_zone
}; //(6th hex values 0x01 to 0x09 : examples: 01 = Mode1, 02 = Mode2, 03 = Mode3 .. 09 = Mode9)

constexpr std::array<uint8_t, 10> V6_Bridge_Mode_Speed_Up{ 0x31, 0x00, 0x00, 0x00, 0x03, 0x02, 0x00, 0x00, 0x00, 0x01 };
constexpr std::array<uint8_t, 10> V6_RGBWW_Mode_Speed_Up{ 0x31, 0x00, 0x00, 0x08, 0x04, 0x03, 0x00, 0x00, 0x00, l_zone };
constexpr std::array<uint8_t, 10> V6_RGBW_Mode_Speed_Up{ 0x31, 0x00, 0x00, 0x07, 0x03, 0x03, 0x00, 0x00, 0x00, l_zone };

constexpr std::array<uint8_t, 10> V6_Bridge_Mode_Speed_Down{ 0x31, 0x00, 0x00, 0x00, 0x03, 0x01, 0x00, 0x00, 0x00, 0x01 };
constexpr std::array<uint8_t, 10> V6_RGBWW_Mode_Speed_Down{ 0x31, 0x00, 0x00, 0x08, 0x04, 0x04, 0x00, 0x00, 0x00, l_zone };
constexpr std::array<uint8_t, 10> V6_RGBW_Mode_Speed_Down{ 0x31, 0x00, 0x00, 0x07, 0x03, 0x04, 0x00, 0x00, 0x00, l_zone };

// V4/V5
// White LEDs
constexpr std::array<unsigned char, 3> AllOn{ 0x35, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> AllOff{ 0x39, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> AllNight{ 0xB9, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> AllFull{ 0xB5, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> BrightnessUp{ 0x3C, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> BrightnessDown{ 0x34, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> ColorTempUp{ 0x3E, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> ColorTempDown{ 0x3F, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> Group1On{ 0x38, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group1Off{ 0x3B, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group1Night{ 0xBB, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group1Full{ 0xB8, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> Group2On{ 0x3D, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group2Off{ 0x33, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group2Night{ 0xB3, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group2Full{ 0xBD, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> Group3On{ 0x37, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group3Off{ 0x3A, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group3Night{ 0xBA, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group3Full{ 0xB7, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> Group4On{ 0x32, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group4Off{ 0x36, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group4Night{ 0xB6, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> Group4Full{ 0xB2, 0x0, 0x55 };

// RGB LEDs
constexpr std::array<unsigned char, 3> RGBOn{ 0x22, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBOff{ 0x21, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> RGBBrightnessUp{ 0x23, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBBrightnessDown{ 0x24, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> RGBDiscoSpeedSlower{ 0x26, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBDiscoSpeedFaster{ 0x25, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBDiscoSpeedFasterLong{ 0xA5, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> RGBDiscoNext{ 0x27, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBDiscoPrevious{ 0x28, 0x0, 0x55 };

std::array<unsigned char, 3> RGBSetColour{ 0x20, 0x0, 0x55 };

//RGBW LEDs
constexpr std::array<unsigned char, 3> RGBWOn{ 0x42, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBWOff{ 0x41, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> RGBWDiscoSpeedSlower{ 0x43, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBWDiscoSpeedFaster{ 0x44, 0x0, 0x55 };

constexpr std::array<unsigned char, 3> RGBWGroup1AllOn{ 0x45, 0x0, 0x55 }; //(SYNC/PAIR RGB+W Bulb within 2 seconds of Wall Switch Power being turned ON)
constexpr std::array<unsigned char, 3> RGBWGroup1AllOff{ 0x46, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBWGroup2AllOn{ 0x47, 0x0, 0x55 }; //(SYNC/PAIR RGB+W Bulb within 2 seconds of Wall Switch Power being turned ON)
constexpr std::array<unsigned char, 3> RGBWGroup2AllOff{ 0x48, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBWGroup3AllOn{ 0x49, 0x0, 0x55 }; //(SYNC/PAIR RGB+W Bulb within 2 seconds of Wall Switch Power being turned ON)
constexpr std::array<unsigned char, 3> RGBWGroup3AllOff{ 0x4A, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBWGroup4AllOn{ 0x4B, 0x0, 0x55 }; //(SYNC/PAIR RGB+W Bulb within 2 seconds of Wall Switch Power being turned ON)
constexpr std::array<unsigned char, 3> RGBWGroup4AllOff{ 0x4C, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBWDiscoMode{ 0x4D, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> RGBWAllNightMode{ 0xC1, 0x0, 0x55 };    // Set All RGBW to Night Mode
constexpr std::array<unsigned char, 3> RGBWGroup1NightMode{ 0xC6, 0x0, 0x55 }; // Set Group 1 to Night Mode
constexpr std::array<unsigned char, 3> RGBWGroup2NightMode{ 0xC8, 0x0, 0x55 }; // Set Group 2 to Night Mode
constexpr std::array<unsigned char, 3> RGBWGroup3NightMode{ 0xCA, 0x0, 0x55 }; // Set Group 3 to Night Mode
constexpr std::array<unsigned char, 3> RGBWGroup4NightMode{ 0xCC, 0x0, 0x55 }; // Set Group 4 to Night Mode

constexpr std::array<unsigned char, 3> RGBWSetColorToWhiteAll{ 0xC2, 0x0, 0x55 };    // SET COLOR TO WHITE (GROUP ALL) 0x42 100ms followed by: 0xC2
constexpr std::array<unsigned char, 3> RGBWSetColorToWhiteGroup1{ 0xC5, 0x0, 0x55 }; // SET COLOR TO WHITE (GROUP 1) 0x45 100ms followed by: 0xC5
constexpr std::array<unsigned char, 3> RGBWSetColorToWhiteGroup2{ 0xC7, 0x0, 0x55 }; // SET COLOR TO WHITE (GROUP 2) 0x47 100ms followed by: 0xC7
constexpr std::array<unsigned char, 3> RGBWSetColorToWhiteGroup3{ 0xC9, 0x0, 0x55 }; // SET COLOR TO WHITE (GROUP 3) 0x49 100ms followed by: 0xC9
constexpr std::array<unsigned char, 3> RGBWSetColorToWhiteGroup4{ 0xCB, 0x0, 0x55 }; // SET COLOR TO WHITE (GROUP 4) 0x4B 100ms followed by: 0xCB

std::array<unsigned char, 3> RGBWSetBrightnessLevel{ 0x4E, 0, 0x55 };
...
std::array<unsigned char, 3> RGBWSetColor{ 0x40, 0, 0x55 };
...
//White LEDs
constexpr std::array<unsigned char, 3> WhiteBrightnessUp{ 0x3C, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> WhiteBrightnessDown{ 0x34, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> WhiteWarmer{ 0x3E, 0x0, 0x55 };
constexpr std::array<unsigned char, 3> WhiteCooler{ 0x3F, 0x0, 0x55 };
...
bool CLimitLess::SendV6Command(const uint8_t* pCmd)
{
	...
	uint8_t RBuffer[100];
	int wPointer = 0;
	memcpy(OutBuffer + wPointer, V6_PreAmble.data(), V6_PreAmble.size());
	wPointer += V6_PreAmble.size();
