...
//		SwitchSubType				= atoi((*row)[3].c_str() );
		...
		if (SwitchIdx<=0){
 				_log.Log(LOG_ERROR,"No Switch device associted to Thermostat  name =%s", ThermostatName);
		}
		else if (ThermostatSetPoint == 0)
		{
			if ((Minute % 10) == 0 )
					_log.Debug(DEBUG_NORM, "VTHER: Mn:%02d  Therm:%-10s(%2d) SetPoint:%4.1f POWER OFF LightId(%2ld):%d ", Minute, ThermostatName, ThermostatId , ThermostatSetPoint, SwitchIdx, SwitchValue);
		...
		else if ( atoi(TemperatureId.c_str()) > 0  )
		{
			...
			if ( GetLastValue( TemperatureId.c_str(), nValue, sValue,  LastUpdateTime) )
			{
                ...
                double minute   =  double(Minute)+double(i)*scrollDevice ;
				SwitchValue     = ComputeThermostatOutput ( (int)minute ,PowerModulation);
			    ...
			    TSqlQueryResult resSw = m_sql.safe_query("SELECT nValue,Type,SubType,SwitchType,LastLevel,Name FROM DeviceStatus WHERE (ID == %s )", SwitchIdxStr.c_str()  ) ;

				if (resSw.size())
				{
					//SwitchSubType    = atoi(resSw[0][2].c_str());
					lastSwitchValue	 = atoi(resSw[0][0].c_str() )  ;
                    switchtype       = atoi(resSw[0][3].c_str());
                    int LastLevel    = atoi(resSw[0][4].c_str());
    				...
    				//		TSqlQueryResult res=m_sql.safe_query("SELECT ID FROM DeviceStatus where Name=='his_%s'", ThermostatName ) ;
			...
			_log.Log(LOG_ERROR,"No temperature device associted to Thermostat name =%s", ThermostatName);
	}//nd for
}
catch (...)
{
...
float VirtualThermostat::GetSetPointTemperature(std::string &devIdx)
{
	...
	return ((float)atof( GetSetPoint(devIdx).c_str()  ) );
} ;
...
bool VirtualThermostat::GetLastValue(  const char* DeviceID, int &nValue, std::string &sValue, struct tm &LastUpdateTime)
{
	...
	sqlresult=m_sql.safe_query(szTmp);
