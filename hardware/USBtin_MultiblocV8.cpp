...
bool USBtin_MultiblocV8::StartThread()
{
	...
	min_counter = (60*5);
	min_counter2 = (3600*6);
	_log.Log(LOG_NORM,"MultiblocV8: clearing BlocList Table on startup");
	ClearingBlocList();
...
void USBtin_MultiblocV8::StopThread()
{
	...
	_log.Log(LOG_NORM,"MultiblocV8: clearing BlocList Table on shutdown");
	ClearingBlocList();
}
...
void USBtin_MultiblocV8::SendRequest(int sID ){
	...
	szTrameToSend += "8";
	if( BOOL_DebugInMultiblocV8 == true ) _log.Log(LOG_NORM,"MultiblocV8: write frame to Can: #%s# ",szTrameToSend.c_str());
	writeFrame(szTrameToSend);
	usleep(TIME_10ms); //wait time to not overrun can bus if many request are send...
}

void USBtin_MultiblocV8::Traitement_MultiblocV8(int IDhexNumber,unsigned int rxDLC, unsigned int Buffer_Octets[8],bool BOOL_Debug){
	...
	BOOL_DebugInMultiblocV8 = BOOL_Debug;
...
void USBtin_MultiblocV8::Traitement_SFSP_Switch_Recu(const unsigned int FrameType,const unsigned char RefBloc, const char Codage, const char Ssreseau, unsigned int bufferdata[8])
{
	...
	switch(RefBloc){ //Switch on ref bloc to find the bloc product who answer
		case BLOC_SFSP_M :
...
bool USBtin_MultiblocV8::CheckOutputChange(unsigned long sID,char Subtype,int LightingType,int OutputNumber,int CdeReceive,int CommandPWM,int LevelReceive){
	char szDeviceID[10];
	int i;
	...
	int StoreIdToFind = sID &(MSK_INDEX_MODULE+MSK_CODAGE_MODULE+MSK_SRES_MODULE);
	...
	result = m_sql.safe_query("SELECT ID,nValue,sValue FROM DeviceStatus WHERE (HardwareID==%d) AND (DeviceID=='%q') AND (Type==%d) AND (SubType==%d) AND (Unit==%d)", 
			m_HwdID, szDeviceID, LightingType, Subtype, OutputNumber); //Unit = 1 = sortie n°1
...
void USBtin_MultiblocV8::OutputNewStates(unsigned long sID,char Subtype,int OutputNumber,int Comandeonoff,int CommandPWM,int Level ){
	...
	lcmd.LIGHTING2.rssi = 12;
	//default name creation :
	std::string defaultname="output S";
	std::ostringstream convert;   // stream used for the conversion
...
void USBtin_MultiblocV8::Traitement_Etat_S_TOR_Recu(const unsigned int FrameType,const unsigned char RefeBloc, const char Codage, const char Ssreseau,unsigned int bufferdata[8])
{
	...
	switch(FrameType){
		case type_STATE_S_TOR_1_TO_2:
			...
			level_value /= 16;
			if( level_value > 15 ) level_value = 15;
			if( CheckOutputChange(sID,Subtype,pTypeLighting2,1,OutputCde,OutputPWM,level_value) == true ) { //on met à jours si nécessaire !
				OutputNewStates( sID, Subtype, 1,OutputCde,OutputPWM,level_value );
			}
			...
			level_value /= 16;
			if( level_value > 15 ) level_value = 15;
			if( CheckOutputChange(sID,Subtype,pTypeLighting2,2,OutputCde,OutputPWM,level_value) == true ) { //on met à jours si nécessaire !
				OutputNewStates( sID, Subtype, 2,OutputCde,OutputPWM,level_value );
			}
			break;
		...
		case type_STATE_S_TOR_3_TO_4:
			...
			level_value /= 16;
			if( level_value > 15 ) level_value = 15;
			if( CheckOutputChange(sID,Subtype,pTypeLighting2,3,OutputCde,OutputPWM,level_value) == true ) { //on met à jours si nécessaire !
				OutputNewStates( sID, Subtype, 3,OutputCde,OutputPWM,level_value );
			}
			...
			level_value /= 16;
			if( level_value > 15 ) level_value = 15;
			if( CheckOutputChange(sID,Subtype,pTypeLighting2,4,OutputCde,OutputPWM,level_value) == true ) { //on met à jours si nécessaire !
				OutputNewStates( sID, Subtype, 4,OutputCde,OutputPWM,level_value );
			}
			break;
		...
		case type_STATE_S_TOR_5_TO_6:
			...
			level_value /= 16;
			if( level_value > 15 ) level_value = 15;
			if( CheckOutputChange(sID,Subtype,pTypeLighting2,5,OutputCde,OutputPWM,level_value) == true ) { //on met à jours si nécessaire !
				OutputNewStates( sID, Subtype, 5,OutputCde,OutputPWM,level_value );
			}
			...
			level_value /= 16;
			if( level_value > 15 ) level_value = 15;
			if( CheckOutputChange(sID,Subtype,pTypeLighting2,6,OutputCde,OutputPWM,level_value) == true ) { //on met à jours si nécessaire !
				OutputNewStates( sID, Subtype, 6,OutputCde,OutputPWM,level_value );
			}
			break;
	}
...
bool USBtin_MultiblocV8::WriteToHardware(const char *pdata, const unsigned char length)
{
	...
	if (packettype == pTypeLighting2 && ( pSen->LIGHTING2.subtype == sTypeSFSP_M || pSen->LIGHTING2.subtype == sTypeSFSP_E ))
	{
		//light command
