	...
	if (packettype == pTypeGeneralSwitch) {
			 ...
			 ((xcmd->subtype == sSwitchGeneralSwitch) && (xcmd->unitcode == XiaomiUnitCode::GATEWAY_SOUND_MP3)))
		{
			...
			message = R"({"cmd":"write","model":"gateway","sid":")" + m_GatewaySID + R"(","short_id":0,"data":"{\"mid\":)" +
				  m_GatewayMusicId + R"(,\"vol\":)" + m_GatewayVolume + R"(,\"key\":\"@gatewaykey\"}" })";
		}
			...
			if ((cmd == COMMAND_REPORT) || (cmd == COMMAND_READ_ACK) || (cmd == COMMAND_HEARTBEAT))
			{
				...
				if ((ret) || (!root2.isObject()))
				{
					...
					if (type != STYPE_END)
					{
						...
						if ((status == STATE_MOTION_YES) || (status == STATE_OPEN) || (status == "no_close") ||
						    (status == STATE_ON) || (!no_close.empty()))
						{
							level = 0;
							on = true;
						}
						else if ((status == STATE_MOTION_NO) || (status == STATE_CLOSE) || (status == STATE_OFF) ||
							 (!no_motion.empty()))
						{
							level = 0;
							on = false;
