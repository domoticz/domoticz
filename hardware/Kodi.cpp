	...
	try
	{
		if (!IsStopRequested(0) && !m_Socket)
		{
			...
			if (!ec)
			{
				...
				m_Socket->async_read_some(boost::asio::buffer(m_Buffer, sizeof m_Buffer), [this, self](boost::system::error_code err, size_t bytes) { handleRead(err, bytes); });
...
void CKodiNode::handleRead(const boost::system::error_code& e, std::size_t bytes_transferred)
{
	if (!e)
	{
		...
		{
			auto self = shared_from_this();
			m_Socket->async_read_some(boost::asio::buffer(m_Buffer, sizeof m_Buffer), [this, self](const boost::system::error_code &err, size_t bytes) { handleRead(err, bytes); });
		}
	}
	else
	{
