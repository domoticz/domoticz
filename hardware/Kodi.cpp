...
void CKodi::ReloadNodes()
{
	...
	if (!result.empty())
	{
		...
		for (auto sd : result) {
			auto pNode = std::make_shared<CKodiNode>(&m_ios, m_HwdID, m_iPollInterval, m_iPingTimeoutms, sd[0], sd[1], sd[2],
								 sd[3]);
		...
		for (auto &m_pNode : m_pNodes) {
			...
			auto tAsync = new boost::thread(&CKodiNode::Do_Work, m_pNode);
			SetThreadName(tAsync->native_handle(), "KodiNode");
		}
