...
void CHEOS::ParseLine()
{
	...
	try
	{
		...
		_log.Debug(DEBUG_HARDWARE, "HEOS: Handling message: '%s'.", sLine.c_str());

		bool bRetVal = ParseJSon(sLine, root);
		if ((!bRetVal) || (!root.isObject()))
		{
			_log.Log(LOG_ERROR, "HEOS: PARSE ERROR: '%s'", sLine.c_str());
		}
		else
		{
			...
			if (root.isMember("heos"))
			{
				if (root["heos"].isMember("result"))
				{
							...
							else if (root["heos"]["command"] == "player/get_players")
							{
								if (root.isMember("payload"))
								{
									for (const auto &r : root["payload"])
									{
										...
										{
											_log.Debug(DEBUG_HARDWARE, "HEOS: No players found.");
										}
									}
								}
								else
								{
									_log.Debug(DEBUG_HARDWARE, "HEOS: No players found (No Payload).");
								}
							}
					...
					else
					{
						if (root["heos"].isMember("command"))
						{
							_log.Debug(DEBUG_HARDWARE, "HEOS: Failed: '%s'.", root["heos"]["command"].asCString());
						}
					}
				}
				else
			...
			{
				_log.Debug(DEBUG_HARDWARE, "HEOS: Message not generated by HEOS System.");
			}
	...
	{
		_log.Log(LOG_ERROR, "HEOS: Exception: %s", e.what());
	}
}
...
void CHEOS::SendCommand(const std::string &command)
{
	...
	if (sMessage.length())
	{
		if (WriteInt(sMessage))
		{
			if (systemCall)
			{
				_log.Debug(DEBUG_HARDWARE, "HEOS: Sent command: '%s'.", sMessage.c_str());
			}
			else
			{
				_log.Log(LOG_NORM, "HEOS: Sent command: '%s'.", sMessage.c_str());
			}
		}
	}
	else
	{
		_log.Log(LOG_ERROR, "HEOS: Command: '%s'. Unknown command.", command.c_str());
	}
}
...
void CHEOS::SendCommand(const std::string &command, const int iValue)
{
	...
	*/
	...
	/* Process */
	_log.Debug(DEBUG_HARDWARE, "HEOS: Debug: '%s'.", sMessage.c_str());
	...
	if (sMessage.length())
	{
		...
		{
			_log.Debug(DEBUG_HARDWARE, "HEOS: Not Connected - Message not sent: '%s'.", sMessage.c_str());
		}
	}
	else
	{
...
void CHEOS::Do_Work()
{
	...
	_log.Log(LOG_STATUS, "HEOS: Worker started...");
	...
	_log.Log(LOG_STATUS, "HEOS: Worker stopped...");
...
{
	_log.Log(LOG_STATUS, "HEOS: Connected to: %s:%d", m_IP.c_str(), m_usIPPort);
	m_bIsStarted = true;
	m_bufferpos = 0;
	sOnConnected(this);
}
...
{
	_log.Log(LOG_STATUS, "HEOS: Disconnected");
}
...
void CHEOS::OnError(const boost::system::error_code& error)
{
	...
	{
		_log.Log(LOG_ERROR, "HEOS: Can not connect to: %s:%d", m_IP.c_str(), m_usIPPort);
	}
	else if (
	...
	{
		_log.Log(LOG_STATUS, "HEOS: Connection reset!");
	}
	else
		_log.Log(LOG_ERROR, "HEOS: %s", error.message().c_str());
}
...
void CHEOS::ReloadNodes()
{
	...
	if (!result.empty())
	{
		_log.Log(LOG_STATUS, "HEOS: %d players found.", (int)result.size());
	...
	{
		_log.Log(LOG_ERROR, "HEOS: No players found.");
	}
}
