...
bool COpenWebNetTCP::StartHardware()
{
	...
	m_monitorThread = std::make_shared<std::thread>(std::bind(&COpenWebNetTCP::MonitorFrames, this));

	//Start worker thread
	if (m_monitorThread != NULL) {
		m_heartbeatThread = std::make_shared<std::thread>(std::bind(&COpenWebNetTCP::Do_Work, this));
	}
