...
namespace Plugins {
	...
	void CPluginProtocol::ProcessInbound(const ReadMessage* Message)
	{
		...
		ReceivedMessage*	RecvMessage = new ReceivedMessage(Message->m_HwdID, Message->m_pConnection, Message->m_Buffer);
		{
	...
	void CPluginProtocol::Flush(const int HwdID, PyObject* pConnection)
	{
		// Forced buffer clear, make sure the plugin gets a look at the data in case it wants it
		ReceivedMessage*	RecvMessage = new ReceivedMessage(HwdID, pConnection, m_sRetainedData);
		{
			boost::lock_guard<boost::mutex> l(PluginMutex);
			PluginMessageQueue.push(RecvMessage);
		}
	...
	void CPluginProtocolLine::ProcessInbound(const ReadMessage* Message)
	{
		...
		{
			ReceivedMessage*	RecvMessage = new ReceivedMessage(Message->m_HwdID, Message->m_pConnection, std::vector<byte>(&sData[0], &sData[iPos]));
			{
				boost::lock_guard<boost::mutex> l(PluginMutex);
				PluginMessageQueue.push(RecvMessage);
			}
	...
	void CPluginProtocolJSON::ProcessInbound(const ReadMessage* Message)
	{
		...
		while (iPos) {
			...
			if (!iPos) // no, just one or part of one
			{
				...
				{
					ReceivedMessage*	RecvMessage = new ReceivedMessage(Message->m_HwdID, Message->m_pConnection, sData);
					{
						boost::lock_guard<boost::mutex> l(PluginMutex);
						PluginMessageQueue.push(RecvMessage);
					}
			...
			else  // more than one message so queue the first one
			{
				...
				ReceivedMessage*	RecvMessage = new ReceivedMessage(Message->m_HwdID, Message->m_pConnection, sMessage);
				{
					boost::lock_guard<boost::mutex> l(PluginMutex);
					PluginMessageQueue.push(RecvMessage);
				}
		...
		try
		{
			while (true)
			{
				...
				if (iPos != std::string::npos)
				{
					...
					ReceivedMessage*	RecvMessage = new ReceivedMessage(Message->m_HwdID, Message->m_pConnection, sData.substr(0, iEnd));
					{
						boost::lock_guard<boost::mutex> l(PluginMutex);
						PluginMessageQueue.push(RecvMessage);
					}
		...
		if (m_Status)
		{
			if (!m_Chunked)
			{
				...
				if (m_ContentLength == sData.length())
				{
					...
					ReceivedMessage*	RecvMessage = new ReceivedMessage(Message->m_HwdID, Message->m_pConnection, vData, m_Status, (PyObject*)m_Headers);
					boost::lock_guard<boost::mutex> l(PluginMutex);
					PluginMessageQueue.push(RecvMessage);
					m_sRetainedData.clear();
				}
			...
			else
			{
				...
				while (sData.length() && (sData != "\r\n"))
				{
					if (!m_RemainingChunk)	// Not processing a chunk so we should be at the start of one
					{
						...
						{
							ReceivedMessage*	RecvMessage = new ReceivedMessage(Message->m_HwdID, Message->m_pConnection, vHTML, m_Status, (PyObject*)m_Headers);
							boost::lock_guard<boost::mutex> l(PluginMutex);
							PluginMessageQueue.push(RecvMessage);
							m_sRetainedData.clear();
							break;
