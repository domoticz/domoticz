	...
	int CDevice_init(CDevice *self, PyObject *args, PyObject *kwds)
	{
		...
		try
		{
					...
					while(PyDict_Next(Options, &pos, &pKey, &pValue))
					{
						...
						else
						{
							_log.Log(
								...
								self->pPlugin->m_Name.c_str(), self->HwdID, self->Unit,
			...
			else
			{
				...
				_log.Log(
					LOG_ERROR,
					R"(Expected: myVar = Domoticz.Device(Name="myDevice", Unit=0, TypeName="", Type=0, Subtype=0, Switchtype=0, Image=0, Options={}, Used=1))");
				LogPythonException(pPlugin, __func__);
			}
		}
		catch (std::exception *e)
