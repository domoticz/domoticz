...
namespace Plugins
{
	...
	extern void maptypename(const std::string &sTypeName, int &Type, int &SubType, int &SwitchType, std::string &sValue, PyObject *OptionsIn, PyObject *OptionsOut)
	{
		...
		if (sTypeName == "Pressure")
			SubType = sTypePressure;
		else if (sTypeName == "Percentage")
		...
		else if (sTypeName == "Voltage")
			SubType = sTypeVoltage;
		else if (sTypeName == "Text")
		...
		else if (sTypeName == "Visibility")
			SubType = sTypeVisibility;
		else if (sTypeName == "Distance")
			SubType = sTypeDistance;
		else if (sTypeName == "Counter Incremental")
			SubType = sTypeCounterIncremental;
		else if (sTypeName == "Soil Moisture")
			SubType = sTypeSoilMoisture;
		else if (sTypeName == "Leaf Wetness")
			SubType = sTypeLeafWetness;
		else if (sTypeName == "kWh")
		{
			sValue = "0; 0.0";
			SubType = sTypeKwh;
		}
		else if (sTypeName == "Current (Single)")
			SubType = sTypeCurrent;
		else if (sTypeName == "Solar Radiation")
			SubType = sTypeSolarRadiation;
	...
	PyObject *CDevice_refresh(CDevice *self)
	{
		if ((self->pPlugin) && (self->HwdID != -1) && (self->Unit != -1))
		{
			...
			Py_END_ALLOW_THREADS if (!result.empty())
			{
				for (const auto &sd : result)
				{
	...
	PyObject *CDevice_update(CDevice *self, PyObject *args, PyObject *kwds)
	{
		if (self->pPlugin)
		{
			...
			std::string sDeviceID = PyUnicode_AsUTF8(self->DeviceID);
			std::string sDescription = PyUnicode_AsUTF8(self->Description);
			static char *kwlist[] = {"nValue", "sValue", "Image", "SignalLevel", "BatteryLevel", "Options", "TimedOut",
	...
	{
		Py_BEGIN_ALLOW_THREADS if ((self->pPlugin) && (self->HwdID != -1) && (self->Unit != -1))
		{
			self->pPlugin->SetHeartbeatReceived();
			std::string sID = std::to_string(self->ID);
		...
		Py_END_ALLOW_THREADS return CDevice_refresh(self);
	}
