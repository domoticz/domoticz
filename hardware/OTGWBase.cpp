...
void OTGWBase::ParseLine()
{
	...
	if (results.size()==25)
	{
		...
		_status.DHW_setpoint = static_cast<float>(atof(results[idx++].c_str())); if(_status.DHW_setpoint!=0.0f)							UpdateSetPointSensor(idx - 1, _status.DHW_setpoint, "DHW Setpoint");
