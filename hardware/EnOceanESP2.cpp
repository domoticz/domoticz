...
{
	constexpr std::array<std::pair<unsigned long, const char *>, 112> TTable{
		{
			{ 0x001, "Peha" },
			{ 0x002, "Thermokon" },
			{ 0x003, "Servodan" },
	...
	auto it = std::find_if(TTable.begin(), TTable.end(), [=](auto &&t) { return t.first == ID; });
	return it != TTable.end() ? it->second : ">>Unkown... Please report!<<";
}
...
constexpr std::array<_t4BSLookup, 104> T4BSTable{
	{
...
{
	auto it = std::find_if(T4BSTable.begin(), T4BSTable.end(), [=](auto &&t) { return t.Org == Org && t.Func == Func && t.Type == Type; });
	return it != T4BSTable.end() ? it->Label : ">>Unkown... Please report!<<";
}

const char* Get_Enocean4BSDesc(const int Org, const int Func, const int Type)
{
	auto it = std::find_if(T4BSTable.begin(), T4BSTable.end(), [=](auto &&t) { return t.Org == Org && t.Func == Func && t.Type == Type; });
	return it != T4BSTable.end() ? it->Description : ">>Unkown... Please report!<<";
}
