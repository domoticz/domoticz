		...
		if (pChild->GetValue(vType, intValue))
		{
			...
			else
			{
				if (pChild->GetValue(V_FORECAST, stringValue))
				{
					...
					sstr << pChild->nodeID;
					m_sql.UpdateValue(m_HwdID, sstr.str().c_str(), pChild->childID, pTypeGeneral, sTypeTextStatus, 12, pChild->batValue, 0, stringValue.c_str(), (!pChild->childName.empty()) ? pChild->childName : "Forecast");
				}
			}
		}
		break;
