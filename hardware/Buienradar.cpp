...
bool CBuienRadar::GetStationDetails()
{
	...
	if (m_iStationID==0) {
		...
		_log.Log(LOG_STATUS, "BuienRadar: Selected station: %s (%s), ID: %d at %s,%s", m_sStationName.c_str(), m_sStationRegion.c_str(), m_iStationID, m_szMyLatitude.c_str(), m_szMyLongitude.c_str());
