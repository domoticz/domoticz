...
size_t unzGetFileList(unzFile file, std::vector<std::string> &result)
{
	if (file==NULL)
		return UNZ_PARAMERROR;
	result.clear();
	...
	for (i=0;i<gi.number_entry;i++)
	{
		...
		unz_file_info file_info;
		err = unzGetCurrentFileInfo(file,&file_info,filename_inzip,sizeof(filename_inzip),NULL,0,NULL,0);
		if (err!=UNZ_OK)
		{
			//printf("error %d with zipfile in unzGetCurrentFileInfo\n",err);
			break;
		}
		if (strlen(filename_inzip)>1)
		{
...
int do_extract_currentfile( unzFile uf,const char* password,char *szOutputPath)
{
	char filename_inzip[256];
	...
	{
		sprintf(write_filename,"%s%s",szOutputPath,filename_inzip);
