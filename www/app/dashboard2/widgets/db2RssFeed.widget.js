define([
    ...
    app.directive('db2RssFeedWidget', [function() {
        return {
            restrict:         'E',
            templateUrl:      'views/dashboard2/widgets/rss-feed.html',
            scope: {
                widgetDef: '=',
                editMode:  '<'
            },
            ...
            controller: ['$scope', '$http', '$interval', '$q', function($scope, $http, $interval, $q) {
                ...
                function fetchViaProxy(cfg, maxItems, proxyIndex) {
                    ...
                    var proxyUrl = proxies[idx] + encodeURIComponent(cfg.feedUrl);
                    return $http.get(proxyUrl, {
                        timeout:     cancelToken ? cancelToken.promise : undefined,
                        transformResponse: function(data) { return data; }
                    }).then(function(resp) {
                        ctrl.loading = false;
                        var xml = (typeof resp.data === 'string') ? resp.data : (resp.data && resp.data.contents);
                        if (!xml) { ctrl.loadError = true; return; }
