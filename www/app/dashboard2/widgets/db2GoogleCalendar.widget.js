define([
    ...
    app.directive('db2GoogleCalendarWidget', [function() {
        return {
            restrict:         'E',
            templateUrl:      'views/dashboard2/widgets/google-calendar.html',
            scope: {
                widgetDef: '=',
                editMode:  '<'
            },
            ...
            controller: ['$scope', '$http', '$interval', '$q', function($scope, $http, $interval, $q) {
                ...
                function fetchAndParse(cfg, proxyIndex) {
                    ...
                    var url   = ICS_PROXIES[idx] ? ICS_PROXIES[idx] + encodeURIComponent(cfg.calendarUrl) : cfg.calendarUrl;
                    var isICS = /\.ics(\?.*)?$/i.test(cfg.calendarUrl) || idx > 0;
                    ...
                    $http.get(url, {
                        ...
                        ctrl.loading = false;
                        var text = typeof resp.data === 'string' ? resp.data : JSON.stringify(resp.data);
                        var events;
