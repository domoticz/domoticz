define([ "app" ], function(app) {
    app.controller("DPHttpController", [ "$scope", "$rootScope", "$location", "$http", "$interval", function($scope, $rootScope, $location, $http, $interval) {
        ...
        SaveConfiguration = function() {
            ...
            $.ajax({
                url: "json.htm?type=command&param=savehttplinkconfig" + "&url=" + encodeURIComponent(cleanurl) + "&method=" + method + "&headers=" + encodeURIComponent(httpheaders) + "&data=" + encodeURIComponent(httpdata) + "&linkactive=" + linkactive + "&debugenabled=" + debugenabled + "&auth=" + auth + "&authbasiclogin=" + authbasiclogin + "&authbasicpassword=" + authbasicpassword,
                async: false,
                dataType: "json",
                ...
                success: function(data) {
                    if (typeof data.result != "undefined") {
                        $.each(data.result, function(i, item) {
                            ...
                            if (item.Enabled == 1) enabled = $.t("Yes");
                            if (item.IncludeUnit == 1) includeUnit = $.t("Yes");
                            var TargetType = $.t("Global variable");
                            if (item.TargetType == 1) TargetType = $.t("Virtual device");
                            ...
                            if (item.TargetDevice == 0) item.TargetDevice = "";
                            var DelimitedValue = "";
                            if (item.TargetType == 2) {
