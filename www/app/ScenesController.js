    ...
    app.controller("ScenesController", [ "$scope", "$rootScope", "$location", "$http", "$interval", "permissions", function($scope, $rootScope, $location, $http, $interval, permissions) {
        ...
        SaveScene = function() {
            ...
            if (bValid) {
                ...
                $.ajax({
                    url: "json.htm?type=updatescene&idx=" + $.devIdx + "&scenetype=" + SceneType + "&name=" + encodeURIComponent($("#scenecontent #devicename").val()) + "&description=" + encodeURIComponent($("#scenecontent #devicedescription").val()) + "&onaction=" + btoa(onaction) + "&offaction=" + btoa(offaction) + "&protected=" + bIsProtected,
                    async: false,
                    dataType: "json",
                    success: function(data) {
                        ShowScenes();
                ...
                onChange: function(hsb, hex, rgb, fromSetColor) {
                    if (!fromSetColor) {
                        ...
                        $.setColValue = setInterval(function() {
                            SetColValue($.lampIdx, hsb.h, hsb.b);
                        }, 400);
                    }
                }
            });
        ...
        GetTimerSettings = function() {
            ...
            if (everyday == true) tsettings.days = 128; else if (weekdays == true) tsettings.days = 256; else if (weekends == true) tsettings.days = 512; else {
                if ($("#scenecontent #timerparamstable #ChkMon").is(":checked")) tsettings.days |= 1;
                if ($("#scenecontent #timerparamstable #ChkTue").is(":checked")) tsettings.days |= 2;
                if ($("#scenecontent #timerparamstable #ChkWed").is(":checked")) tsettings.days |= 4;
                if ($("#scenecontent #timerparamstable #ChkThu").is(":checked")) tsettings.days |= 8;
                if ($("#scenecontent #timerparamstable #ChkFri").is(":checked")) tsettings.days |= 16;
                if ($("#scenecontent #timerparamstable #ChkSat").is(":checked")) tsettings.days |= 32;
                if ($("#scenecontent #timerparamstable #ChkSun").is(":checked")) tsettings.days |= 64;
            }
        ...
        UpdateTimer = function(idx) {
            ...
            $.ajax({
                url: "json.htm?type=command&param=updatescenetimer&idx=" + idx + "&active=" + tsettings.Active + "&timertype=" + tsettings.timertype + "&date=" + tsettings.date + "&hour=" + tsettings.hour + "&min=" + tsettings.min + "&randomness=" + tsettings.Randomness + "&command=" + tsettings.cmd + "&level=" + tsettings.level + "&days=" + tsettings.days + "&mday=" + tsettings.mday + "&month=" + tsettings.month + "&occurence=" + tsettings.occurence,
                async: false,
                dataType: "json",
                success: function(data) {
                    RefreshTimerTable($.devIdx);
        ...
        AddTimer = function() {
            ...
            $.ajax({
                url: "json.htm?type=command&param=addscenetimer&idx=" + $.devIdx + "&active=" + tsettings.Active + "&timertype=" + tsettings.timertype + "&date=" + tsettings.date + "&hour=" + tsettings.hour + "&min=" + tsettings.min + "&randomness=" + tsettings.Randomness + "&command=" + tsettings.cmd + "&level=" + tsettings.level + "&days=" + tsettings.days + "&mday=" + tsettings.mday + "&month=" + tsettings.month + "&occurence=" + tsettings.occurence,
                async: false,
                dataType: "json",
                success: function(data) {
                    RefreshTimerTable($.devIdx);
                ...
                if ($(this).hasClass("row_selected")) {
                    ...
                    if (anSelected.length !== 0) {
                        ...
                        if (data["OrgDayString"] == "Everyday") {
                            ...
                            disableDays = true;
                        } else $("#scenecontent #timerparamstable #when_4").prop("checked", "checked");
        ...
        ShowScenes = function() {
            ...
            var tophtm = "";
            if (permissions.hasPermission("Admin")) {
                tophtm += '	<table border="0" cellpadding="0" cellspacing="0" width="100%">' + "	<tr>" + '	  <td align="left" valign="top" id="timesun"></td>\n' + '	  <td align="right">' + '	    <a class="btnstyle" onclick="AddScene();" data-i18n="Add Scene">Add Scene</a>' + "	  </td>" + "	</tr>" + "	</table>";
            }
            var i = 0;
            var j = 0;
            $.ajax({
