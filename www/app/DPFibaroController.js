define([ "app" ], function(app) {
    app.controller("DPFibaroController", [ "$scope", "$rootScope", "$location", "$http", "$interval", function($scope, $rootScope, $location, $http, $interval) {
        ...
        SaveConfiguration = function() {
            ...
            $.ajax({
                url: "json.htm?type=command&param=savefibarolinkconfig" + "&remote=" + encodeURIComponent(cleanurl) + "&username=" + encodeURIComponent(username) + "&password=" + encodeURIComponent(password) + "&linkactive=" + linkactive + "&isversion4=" + isversion4 + "&debugenabled=" + debugenabled,
                async: false,
                dataType: "json",
