    ...
    app.controller("CustomIconsController", [ "$scope", "$rootScope", "$location", "$http", "$interval", function($scope, $rootScope, $location, $http, $interval) {
        ...
        $scope.uploadFileToUrl = function(file, uploadUrl) {
            ...
            $http.post(uploadUrl, fd, {
                ...
                if (data.status != "OK") {
                    HideNotify();
                    ShowNotify($.t("Error uploading Iconset") + ": " + data.error, 5e3, true);
                }
                $scope.RefreshIconList();
            }).error(function(data) {
                HideNotify();
                ShowNotify($.t("Error uploading Iconset"), 5e3, true);
            });
        };
        ...
        $scope.RefreshIconList = function() {
            $scope.iconset = [];
            $scope.selectedIcon = [];
            $http.get("json.htm?type=command&param=getcustomiconset").success(function(data, status, headers, config) {
        ...
        $scope.UpdateIconTitleDescription = function() {
            ...
            $.ajax({
                url: "json.htm?type=command&param=updatecustomicon&idx=" + $scope.selectedIcon.idx + "&name=" + encodeURIComponent($("#iconname").val()) + "&description=" + encodeURIComponent($("#icondescription").val()),
                async: false,
                dataType: "json",
                success: function(data) {
                    $scope.RefreshIconList();
