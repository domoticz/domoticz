define(['app', 'livesocket', 'widgets/dzLightWidget'], function (app) {
	app.controller('LightsController', function ($scope, $rootScope, $location, $http, $interval, $timeout, $route, $routeParams, deviceApi, permissions, livesocket) {
		...
		RefreshLights = function () {
			...
			livesocket.getJson("json.htm?type=command&param=getdevices&filter=light&used=" + usedFilter + "&order=[Order]&lastupdate=" + $.LastUpdateTime + "&plan=" + roomPlanId, function (data) {
		...
		ShowLights = function () {
			...
			$.ajax({
				url: "json.htm?type=command&param=getdevices&filter=light&used=" + usedFilter + "&order=[Order]&plan=" + roomPlanId,
				dataType: 'json',
				success: function (data) {
					if (typeof data.result != 'undefined') {
						if (typeof data.ActTime != 'undefined') {
