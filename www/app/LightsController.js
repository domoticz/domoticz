define([ "app" ], function(app) {
    app.controller("LightsController", [ "$scope", "$rootScope", "$location", "$http", "$interval", "permissions", function($scope, $rootScope, $location, $http, $interval, permissions) {
        ...
        GetTimerSettings = function() {
            ...
            if (everyday == true) tsettings.days = 128; else if (weekdays == true) tsettings.days = 256; else if (weekends == true) tsettings.days = 512; else {
                if ($("#lightcontent #timerparamstable #ChkMon").is(":checked")) tsettings.days |= 1;
                if ($("#lightcontent #timerparamstable #ChkTue").is(":checked")) tsettings.days |= 2;
                if ($("#lightcontent #timerparamstable #ChkWed").is(":checked")) tsettings.days |= 4;
                if ($("#lightcontent #timerparamstable #ChkThu").is(":checked")) tsettings.days |= 8;
                if ($("#lightcontent #timerparamstable #ChkFri").is(":checked")) tsettings.days |= 16;
                if ($("#lightcontent #timerparamstable #ChkSat").is(":checked")) tsettings.days |= 32;
                if ($("#lightcontent #timerparamstable #ChkSun").is(":checked")) tsettings.days |= 64;
            }
        ...
        UpdateTimer = function(idx) {
            ...
            $.ajax({
                url: "json.htm?type=command&param=updatetimer&idx=" + idx + "&active=" + tsettings.Active + "&timertype=" + tsettings.timertype + "&date=" + tsettings.date + "&hour=" + tsettings.hour + "&min=" + tsettings.min + "&randomness=" + tsettings.Randomness + "&command=" + tsettings.cmd + "&level=" + tsettings.level + "&hue=" + tsettings.hue + "&days=" + tsettings.days + "&mday=" + tsettings.mday + "&month=" + tsettings.month + "&occurence=" + tsettings.occurence,
                async: false,
                dataType: "json",
                success: function(data) {
                    RefreshTimerTable($.devIdx);
        ...
        AddTimer = function() {
            ...
            $.ajax({
                url: "json.htm?type=command&param=addtimer&idx=" + $.devIdx + "&active=" + tsettings.Active + "&timertype=" + tsettings.timertype + "&date=" + tsettings.date + "&hour=" + tsettings.hour + "&min=" + tsettings.min + "&randomness=" + tsettings.Randomness + "&command=" + tsettings.cmd + "&level=" + tsettings.level + "&hue=" + tsettings.hue + "&days=" + tsettings.days + "&mday=" + tsettings.mday + "&month=" + tsettings.month + "&occurence=" + tsettings.occurence,
                async: false,
                dataType: "json",
                success: function(data) {
                    RefreshTimerTable($.devIdx);
                    ...
                    if (typeof data.result != "undefined") {
                        $.each(data.result, function(i, item) {
                            ...
                            if (item.Type <= 4 || item.Type == 8 || item.Type == 9) {
                                ...
                                if (dayflags & 128) DayStrOrig = "Everyday"; else if (dayflags & 256) DayStrOrig = "Weekdays"; else if (dayflags & 512) DayStrOrig = "Weekends"; else {
                                    if (dayflags & 1) {
                ...
                if ($(this).hasClass("row_selected")) {
                    ...
                    if (anSelected.length !== 0) {
                        ...
                        if (data["Days"] == "Everyday") {
                            ...
                            disableDays = true;
                        } else $("#lightcontent #timerparamstable #when_4").prop("checked", "checked");
        ...
        ShowTimers = function(id, name, isdimmer, stype, devsubtype) {
            ...
            $("#lightcontent #picker").colpick({
                ...
                onChange: function(hsb, hex, rgb, fromSetColor) {
                    if (!fromSetColor) {
                        ...
                        $.setColValue = setInterval(function() {
                            SetColValue($.devIdx, hsb.h, hsb.b, bIsWhite);
                        }, 400);
                    }
                }
            });
            ...
            $("#lightcontent #timerparamstable #combocommand").change(function() {
                var cval = $("#lightcontent #timerparamstable #combocommand").val(), lval = $("#lightcontent #timerparamstable #combolevel").val(), bShowLevel = false;
                if (!$.bIsLED) {
                ...
                if (bShowLevel == true) {
                    if (lval == null) {
                        if ($.isSelector) {
                            $("#lightcontent #timerparamstable #combolevel").val(10);
                        } else {
                            $("#lightcontent #timerparamstable #combolevel").val(5);
                        }
                    }
                    $("#lightcontent #LevelDiv").show();
                } else {
            ...
            if ($.isDimmer && !$.bIsLED) {
                ...
                levelDiv$.find("select").attr("style", "").css({
                    width: "auto"
                }).html(html.join(""));
                levelDiv$.find('span[data-i18n="Level"]').attr("data-i18n", "Level name").html($.t("Level name"));
                levelDiv$.show();
            } else {
                $("#lightcontent #LevelDiv").hide();
            }
        ...
        DeleteLightSwitchIntern = function(bRemoveSubDevices) {
            $.ajax({
                url: "json.htm?type=setused&idx=" + $.devIdx + "&name=" + encodeURIComponent($("#lightcontent #devicename").val()) + "&description=" + encodeURIComponent($("#lightcontent #devicedescription").val()) + "&used=false&RemoveSubDevices=" + bRemoveSubDevices,
                async: false,
                dataType: "json",
                success: function(data) {
                    ShowLights();
        ...
        SaveLightSwitch = function() {
            ...
            if ($.bIsSelectorSwitch) {
                var levelNames = unescape($("#lightcontent #selectorlevelstable").data("levelNames")), levelActions = $("#lightcontent #selectoractionstable").data("levelActions"), selectorStyle = $("#lightcontent .selector-switch-options.style input[type=radio]:checked").val(), levelOffHidden = $("#lightcontent .selector-switch-options.level-off-hidden input[type=checkbox]").prop("checked");
            ...
            if (bValid) {
                if ($.stype == "Security") {
                    $.ajax({
                        url: "json.htm?type=setused&idx=" + $.devIdx + "&name=" + encodeURIComponent($("#lightcontent #devicename").val()) + "&description=" + encodeURIComponent($("#lightcontent #devicedescription").val()) + "&strparam1=" + btoa(strParam1) + "&strparam2=" + btoa(strParam2) + "&protected=" + bIsProtected + "&used=true" + "&options=" + btoa(encodeURIComponent(devOptionsParam.join(""))),
                        async: false,
                        dataType: "json",
                        success: function(data) {
                            ShowLights();
                    ...
                    $.ajax({
                        url: "json.htm?type=setused&idx=" + $.devIdx + "&name=" + encodeURIComponent($("#lightcontent #devicename").val()) + "&description=" + encodeURIComponent($("#lightcontent #devicedescription").val()) + "&strparam1=" + btoa(strParam1) + "&strparam2=" + btoa(strParam2) + "&protected=" + bIsProtected + "&switchtype=" + $("#lightcontent #comboswitchtype").val() + "&customimage=" + CustomImage + "&used=true" + addjvalstr + "&options=" + btoa(encodeURIComponent(devOptionsParam.join(""))),
                        async: false,
                        dataType: "json",
                        success: function(data) {
                            ShowLights();
        ...
        ChangeSelectorLevelsOrder = function(from, to) {
            ...
            var table$ = $("#lightcontent #selectorlevelstable"), levelNames = unescape(table$.data("levelNames")).split("|"), fromLevel = levelNames[from], toLevel = levelNames[to];
        ...
        DeleteSelectorLevel = function(index) {
            ...
            var table$ = $("#lightcontent #selectorlevelstable"), levelNames = unescape(table$.data("levelNames")).split("|");
        ...
        RenameSelectorLevel = function(index) {
            ...
            if (index >= 0) {
                var table$ = $("#lightcontent #selectorlevelstable"), levelNames = unescape(table$.data("levelNames")).split("|"), levelName = levelNames[index];
        ...
        AddSelectorLevel = function() {
            var button$ = $("#newselectorlevelbutton"), table$ = $("#lightcontent #selectorlevelstable"), levelName = $("#lightcontent #newselectorlevel").val(), levelNames = unescape(table$.data("levelNames")).split("|");
            levelName = CleanDeviceOptionValue(levelName);
            if (button$.prop("disabled") === true || levelName === "" || $.inArray(levelName, levelNames) !== -1) {
                return;
            }
        ...
        BuildSelectorLevelsTable = function() {
            var table$ = $("#lightcontent #selectorlevelstable"), levelNames = unescape(table$.data("levelNames")).split("|"), levelNamesMaxLength = 11, initializeTable = $("#selectorlevelstable_wrapper").length === 0, oTable = initializeTable ? table$.dataTable({
                iDisplayLength: 25,
                bLengthChange: false,
                bFilter: false,
                bInfo: false,
            ...
            $.each(levelNames, function(index, item) {
                var level = index * 10, updownImg = "", rendelImg = "";
        ...
        UpdateSelectorAction = function(index, levelAction) {
            var table$ = $("#lightcontent #selectoractionstable"), levelActions = table$.data("levelActions").split("|");
        ...
        EditSelectorAction = function(index) {
            ...
            if (index >= 0) {
                var table$ = $("#lightcontent #selectoractionstable"), levelActions = table$.data("levelActions").split("|"), levelAction = levelActions[index];
        ...
        BuildSelectorActionsTable = function() {
            var table$ = $("#lightcontent #selectoractionstable"), levelActions = table$.data("levelActions").split("|"), levelActionsMaxLength = 11, initializeTable = $("#selectoractionstable_wrapper").length === 0, oTable = initializeTable ? table$.dataTable({
                iDisplayLength: 25,
                bLengthChange: false,
                bFilter: false,
                bInfo: false,
            ...
            $.each(levelActions, function(index, item) {
                var level = index * 10, levelAction = unescape(levelActions[index]), rendelImg = "";
        ...
        EditLightDevice = function(idx, name, description, stype, switchtype, addjvalue, addjvalue2, isslave, customimage, devsubtype, strParam1, strParam2, bIsProtected, strUnit) {
            ...
            var oTable;
            if ($.bIsSelectorSwitch) {
                var selectorSwitch$ = $("#selector" + $.devIdx), ssLevelNames = unescape(selectorSwitch$.data("levelnames")), ssStyle = selectorSwitch$.data("selectorstyle"), ssLevelOffHidden = selectorSwitch$.data("leveloffhidden"), ssLevelActions = selectorSwitch$.data("levelactions");
                ...
                $.each($.selectorSwitchLevels, function(index, item) {
                    if (index > $.selectorSwitchActions.length - 1) {
                        $.selectorSwitchActions.push("");
                    }
                });
                if ($.selectorSwitchActions.length > $.selectorSwitchLevels.length) {
            ...
            if ($.stype == "Security") {
                ...
                if (switchtype == 18) {
                    ...
                    dialog_renameselectorlevel_buttons[$.t("Rename")] = function() {
                        var selectorLevelName$ = $("#dialog-renameselectorlevel #selectorlevelname"), selectorIndex$ = $("#dialog-renameselectorlevel #selectorlevelindex"), levelIndex = selectorIndex$.val(), levelName = selectorLevelName$.val(), table$ = $("#lightcontent #selectorlevelstable"), levelNames = unescape(table$.data("levelNames")).split("|"), bValid = true;
                        levelName = CleanDeviceOptionValue(levelName);
                        bValid = bValid && levelIndex >= 0;
                        bValid = bValid && checkLength(selectorLevelName$, 2, 20);
                        bValid = bValid && levelName !== "" && $.inArray(levelName, levelNames) === -1;
                        if (bValid) {
                            $(this).dialog("close");
                    ...
                    dialog_editselectoraction_buttons[$.t("Save")] = function() {
                        var selectorAction$ = $("#dialog-editselectoraction #selectoraction"), selectorIndex$ = $("#dialog-editselectoraction #selectorlevelindex"), levelIndex = selectorIndex$.val(), levelAction = selectorAction$.val().trim(), bValid = true;
                        bValid = bValid && levelIndex >= 0;
                        bValid = bValid && checkLength(selectorAction$, 0, 200);
                        bValid = bValid && (levelAction === "" || (levelAction.toLowerCase().indexOf("http://") === 0 && levelAction.length > 7 || levelAction.toLowerCase().indexOf("https://") === 0 && levelAction.length > 8 || levelAction.toLowerCase().indexOf("script://") === 0 && levelAction.length > 9));
                        if (bValid) {
                            $(this).dialog("close");
                ...
                $.each($.ddData, function(i, item) {
                    if (item.value == customimage) {
                        $("#lightcontent #comboswitchicon").ddslick("select", {
                            index: i
                        });
                    }
                });
        ...
        EvohomePopupMenu = function(item) {
            ...
            $.each([ {
                ...
                htm += '<li><a href="#" class="ui-btn ui-btn-icon-right ui-icon-' + obj.data + '" onclick="SwitchModal(\'' + item.idx + "','" + obj.name + "','" + obj.data + "',RefreshLights);deselect($(this),'#evopop_" + item.idx + "');return false;\">" + obj.name + "</a></li>";
            });
            htm += "</ul></div>";
            return htm;
        };
        ...
        ShowLights = function() {
            ...
            if ($.RoomPlans.length == 0) {
                tophtm += '	<table border="0" cellpadding="0" cellspacing="0" width="100%">\n' + "	<tr>\n" + '	  <td align="left" valign="top" id="timesun"></td>\n' + "	</tr>\n" + "	</table>\n";
            } else {
                tophtm += '	<table border="0" cellpadding="0" cellspacing="0" width="100%">\n' + "	<tr>\n" + '	  <td align="left" valign="top" id="timesun"></td>\n' + '<td align="right">' + '<span data-i18n="Room">Room</span>:&nbsp;<select id="comboroom" style="width:160px" class="combobox ui-corner-all">' + '<option value="0" data-i18n="All">All</option>' + "</select>" + "</td>" + "	</tr>\n" + "	</table>\n";
            }
            if (permissions.hasPermission("Admin")) {
                tophtm += '	<table class="bannav" id="bannav" border="0" cellpadding="0" cellspacing="0" width="100%">\n' + "	<tr>\n" + '	  <td align="left"><a class="btnstylerev" onclick="AddManualLightDevice();" data-i18n="Manual Light/Switch">Manual Light/Switch</a></td>\n' + '	  <td align="right"><a class="btnstyle" onclick="AddLightDevice();" data-i18n="Learn Light/Switch">Learn Light/Switch</a></td>\n' + "	</tr>\n" + "	</table>\n";
            }
            var i = 0;
            var j = 0;
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (typeof data.result != "undefined") {
                        ...
                        $.each(data.result, function(i, item) {
                            ...
                            xhtm += "</td>\n" + "	      <td>";
                            if (item.Favorite == 0) {
                                xhtm += '<img src="images/nofavorite.png" title="' + $.t("Add to Dashboard") + '" onclick="MakeFavorite(' + item.idx + ',1);" class="lcursor">&nbsp;&nbsp;&nbsp;&nbsp;';
                            } else {
                                xhtm += '<img src="images/favorite.png" title="' + $.t("Remove from Dashboard") + '" onclick="MakeFavorite(' + item.idx + ',0);" class="lcursor">&nbsp;&nbsp;&nbsp;&nbsp;';
                            }
                            xhtm += '<a class="btnsmall" onclick="ShowLightLog(' + item.idx + ",'" + escape(item.Name) + "', '#lightcontent', 'ShowLights');\" data-i18n=\"Log\">Log</a> ";
                            if (permissions.hasPermission("Admin")) {
                                xhtm += '<a class="btnsmall" onclick="EditLightDevice(' + item.idx + ",'" + escape(item.Name) + "','" + escape(item.Description) + "', " + "'" + item.Type + "', " + item.SwitchTypeVal + ", " + item.AddjValue + ", " + item.AddjValue2 + ", " + item.IsSubDevice + ", " + item.CustomImage + ", '" + item.SubType + "', '" + item.StrParam1 + "', '" + item.StrParam2 + "'," + item.Protected + "," + item.Unit + ');" data-i18n="Edit">Edit</a> ';
                                if (bAddTimer == true) {
                                    if (item.Timers == "true") {
                                ...
                                if (item.Notifications == "true") xhtm += '<a class="btnsmall-sel" onclick="ShowNotifications(' + item.idx + ",'" + escape(item.Name) + "', '#lightcontent', 'ShowLights'," + bIsDimmer + ",'" + item.Type + "'" + ", '" + item.SubType + '\');" data-i18n="Notifications">Notifications</a>'; else xhtm += '<a class="btnsmall" onclick="ShowNotifications(' + item.idx + ",'" + escape(item.Name) + "', '#lightcontent', 'ShowLights'," + bIsDimmer + ",'" + item.Type + "'" + ", '" + item.SubType + '\');" data-i18n="Notifications">Notifications</a>';
                            }
                            xhtm += "</td>\n" + "	    </tr>\n" + "	    </table>\n" + "	  </section>\n" + "	</div>\n";
                            htmlcontent += xhtm;
                            j += 1;
        ...
        UpdateAddManualDialog = function() {
            ...
            if (lighttype == 0 || lighttype == 1 || lighttype == 3 || lighttype == 101) {
                ...
                tothousecodes = 4;
                totunits = 4;
            } else if (lighttype == 68) {} else if (lighttype == 7) {
                tothousecodes = 16;
                totunits = 8;
            } else if (lighttype == 8) {
                tothousecodes = 16;
                ...
                bIsType5 = 1;
                totunits = 16;
            } else if (lighttype >= 200 && lighttype < 300) {} else if (lighttype == 302) {
                tothousecodes = 4;
                totunits = 4;
            ...
            if (lighttype == 104) {
                ...
                if (bShow1) $("#dialog-addmanuallightdevice #blindsparams #combocmd1").show(); else {
                    $("#dialog-addmanuallightdevice #blindsparams #combocmd1").hide();
                    $("#dialog-addmanuallightdevice #blindsparams #combocmd1").val(0);
                }
                if (bShow4) $("#dialog-addmanuallightdevice #blindsparams #combocmd4").show(); else {
                    $("#dialog-addmanuallightdevice #blindsparams #combocmd4").hide();
                    $("#dialog-addmanuallightdevice #blindsparams #combocmd4").val(0);
                }
                if (bShowUnit) $("#dialog-addmanuallightdevice #blindparamsUnitCode").show(); else $("#dialog-addmanuallightdevice #blindparamsUnitCode").hide();
                $("#dialog-addmanuallightdevice #lighting1params").hide();
                $("#dialog-addmanuallightdevice #lighting2params").hide();
                $("#dialog-addmanuallightdevice #lighting3params").hide();
        ...
        GetManualLightSettings = function(isTest) {
            ...
            if (lighttype == 60) {
                ...
                ID = $("#dialog-addmanuallightdevice #lightparams3 #combocmd1 option:selected").text() + $("#dialog-addmanuallightdevice #lightparams3 #combocmd2 option:selected").text();
                mParams += "&id=" + ID;
            } else if (lighttype == 70) {
                ...
                ID = $("#dialog-addmanuallightdevice #blindsparams #combocmd1 option:selected").text() + $("#dialog-addmanuallightdevice #blindsparams #combocmd2 option:selected").text() + $("#dialog-addmanuallightdevice #blindsparams #combocmd3 option:selected").text() + "0" + $("#dialog-addmanuallightdevice #blindsparams #combocmd4 option:selected").text();
                mParams += "&id=" + ID;
                mParams += "&unitcode=" + $("#dialog-addmanuallightdevice #blindsparams #combounitcode option:selected").val();
            } else if (lighttype == 302) {
                ID = $("#dialog-addmanuallightdevice #homeconfortparams #combocmd1 option:selected").text() + $("#dialog-addmanuallightdevice #homeconfortparams #combocmd2 option:selected").text() + $("#dialog-addmanuallightdevice #homeconfortparams #combocmd3 option:selected").text();
                mParams += "&id=" + ID;
                ...
                ID = $("#dialog-addmanuallightdevice #fanparams #combocmd1 option:selected").text() + $("#dialog-addmanuallightdevice #fanparams #combocmd2 option:selected").text() + $("#dialog-addmanuallightdevice #fanparams #combocmd3 option:selected").text();
                mParams += "&id=" + ID;
            } else if (lighttype == 305) {
                ID = $("#dialog-addmanuallightdevice #fanparams #combocmd1 option:selected").text() + $("#dialog-addmanuallightdevice #fanparams #combocmd2 option:selected").text() + $("#dialog-addmanuallightdevice #fanparams #combocmd3 option:selected").text();
                mParams += "&id=" + ID;
            } else if (lighttype == 400) {
                var appID = parseInt($("#dialog-addmanuallightdevice #openwebnetparamsBus #combocmd1 option:selected").val() + $("#dialog-addmanuallightdevice #openwebnetparamsBus #combocmd2 option:selected").val());
                var ID = "0002" + ("0000" + appID.toString(16)).slice(-4);
                ...
                var ID = "0019" + ("0000" + appID.toString(16)).slice(-4);
                var unitcode = "0";
                mParams += "&id=" + ID.toUpperCase() + "&unitcode=" + unitcode;
            } else if (lighttype == 406) {
                var appID = parseInt($("#dialog-addmanuallightdevice #openwebnetparamsIRdetec #combocmd1 option:selected").val() + $("#dialog-addmanuallightdevice #openwebnetparamsIRdetec #combocmd2 option:selected").val());
                var ID = "0019" + ("0000" + appID.toString(16)).slice(-4);
                var unitcode = "0";
                mParams += "&id=" + ID.toUpperCase() + "&unitcode=" + unitcode;
            } else {
                var ID = "";
                var bIsType5 = 0;
                if (lighttype == 50 || lighttype == 55 || lighttype == 57 || lighttype == 65 || lighttype == 100 || lighttype == 102 || lighttype == 103 || lighttype == 105 || lighttype == 107) {
                    bIsType5 = 1;
                }
                if (bIsType5 == 0) {
                    ID = $("#dialog-addmanuallightdevice #lightparams2 #combocmd1 option:selected").text() + $("#dialog-addmanuallightdevice #lightparams2 #combocmd2 option:selected").text() + $("#dialog-addmanuallightdevice #lightparams2 #combocmd3 option:selected").text() + $("#dialog-addmanuallightdevice #lightparams2 #combocmd4 option:selected").text();
                } else {
                    if (lighttype != 55 && lighttype != 57 && lighttype != 100) {
                        ID = $("#dialog-addmanuallightdevice #lightparams2 #combocmd2 option:selected").text() + $("#dialog-addmanuallightdevice #lightparams2 #combocmd3 option:selected").text() + $("#dialog-addmanuallightdevice #lightparams2 #combocmd4 option:selected").text();
                    } else {
                        ID = $("#dialog-addmanuallightdevice #lightparams2 #combocmd3 option:selected").text() + $("#dialog-addmanuallightdevice #lightparams2 #combocmd4 option:selected").text();
                    }
                }
                if (ID == "") {
                    ...
                    success: function(data) {
                        if (typeof data.status != "undefined") {
                            if (data.status == "OK") {
                                ...
                                ShowNotify(data.message, 3e3, true);
                            }
                        }
                    }
                });
            ...
            $("#dialog-addmanuallightdevice").dialog({
                ...
                open: function() {
                    ...
                    $("#dialog-addmanuallightdevice #lighttable #comboswitchtype").change(function() {
                        var switchtype = $("#dialog-addmanuallightdevice #lighttable #comboswitchtype option:selected").val(), subtype = -1;
                        if (switchtype == 1) {
                            subtype = 10;
                    ...
                    $("#dialog-addmanuallightdevice #lighttable #combolighttype").change(function() {
                        var subtype = $("#dialog-addmanuallightdevice #lighttable #combolighttype option:selected").val(), switchtype = -1;
                        if (subtype == 303) {
                            switchtype = 18;
                        }
                        if (switchtype !== -1) {
