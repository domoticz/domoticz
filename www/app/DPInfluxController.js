define([ "app" ], function(app) {
    app.controller("DPInfluxController", [ "$scope", "$rootScope", "$location", "$http", "$interval", function($scope, $rootScope, $location, $http, $interval) {
        ...
        SaveConfiguration = function() {
            ...
            $.ajax({
                url: "json.htm?type=command&param=saveinfluxlinkconfig" + "&linkactive=" + linkactive + "&remote=" + encodeURIComponent(remoteurl) + "&port=" + port + "&database=" + encodeURIComponent(database) + "&debugenabled=" + debugenabled,
                async: false,
                dataType: "json",
                success: function(data) {
        ...
        AddLink = function(type) {
            ...
            var url = "json.htm?type=command&param=saveinfluxlink" + "&idx=" + idx + "&deviceid=" + deviceid + "&valuetosend=" + valuetosend + "&targettype=" + targettype + "&linkactive=" + linkactive;
            $.ajax({
                url: url,
                async: false,
                dataType: "json",
                ...
                success: function(data) {
                    if (typeof data.result != "undefined") {
                        $.each(data.result, function(i, item) {
                            ...
                            if (item.TargetType == 1) TargetType = $.t("Direct");
