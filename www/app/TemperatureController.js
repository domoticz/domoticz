define([ "app" ], function(app) {
    app.controller("TemperatureController", [ "$scope", "$rootScope", "$location", "$http", "$interval", "$window", "permissions", function($scope, $rootScope, $location, $http, $interval, $window, permissions) {
        ...
        EditSetPoint = function(idx, name, description, setpoint, mode, until, callback) {
            ...
            if (mode.indexOf("Override") == -1) $(":button:contains('Cancel Override')").attr("disabled", "d‌​isabled").addClass("ui-state-disabled"); else $(":button:contains('Cancel Override')").removeAttr("disabled").removeClass("ui-state-disabled");
            $("#dialog-editsetpoint #until").datetimepicker();
            if (until != "") $("#dialog-editsetpoint #until").datetimepicker("setDate", new Date(until));
        ...
        EditState = function(idx, name, description, state, mode, until, callback) {
            ...
            if (until != "") $("#dialog-editstate #until_state").datetimepicker("setDate", new Date(until));
        ...
        EvoSetPointColor = function(item, sHeatMode, bkcolor) {
            if (typeof item.SetPoint != "undefined") {
                if (sHeatMode == "HeatingOff" || item.SetPoint == 325.1) bkcolor = "#9b9b9b"; else if (item.SetPoint >= 25) bkcolor = "#ff0302"; else if (item.SetPoint >= 22) bkcolor = "#ff6a2a"; else if (item.SetPoint >= 19) bkcolor = "#fe9b2d"; else if (item.SetPoint >= 16) bkcolor = "#79bc5c"; else bkcolor = "#6ca5fd";
            }
            return bkcolor;
        };
                            ...
                            ctrl.temperatures.forEach(function(olditem, oldindex, oldarray) {
                                if (olditem.idx == newitem.idx) {
                                    ...
                                    if ($scope.config.ShowUpdatedEffect == true) {
                                        $("#tempwidgets #" + newitem.idx + " #name").effect("highlight", {
                                            color: "#EEFFEE"
                                        }, 1e3);
                                    }
                                }
        ...
        function init() {
            ...
            dialog_edittempdevice_buttons[$.t("Update")] = function() {
                ...
                if (bValid) {
                    ...
                    $.ajax({
                        url: "json.htm?type=setused&idx=" + $.devIdx + "&name=" + encodeURIComponent($("#dialog-edittempdevice #devicename").val()) + "&description=" + encodeURIComponent($("#dialog-edittempdevice #devicedescription").val()) + "&addjvalue=" + aValue + "&used=true",
                        async: false,
                        dataType: "json",
                        success: function(data) {
                            ShowTemps();
            ...
            dialog_edittempdevice_buttons[$.t("Remove Device")] = function() {
                ...
                bootbox.confirm($.t("Are you sure to remove this Device?"), function(result) {
                    if (result == true) {
                        $.ajax({
                            url: "json.htm?type=setused&idx=" + $.devIdx + "&name=" + encodeURIComponent($("#dialog-edittempdevicesmall #devicename").val()) + "&description=" + encodeURIComponent($("#dialog-edittempdevicesmall #devicedescription").val()) + "&used=false",
                            async: false,
                            dataType: "json",
                            success: function(data) {
                                ShowTemps();
            ...
            dialog_editsetpoint_buttons[$.t("Set")] = function() {
                ...
                if (bValid) {
                    $(this).dialog("close");
                    $.ajax({
                        url: "json.htm?type=setused&idx=" + $.devIdx + "&name=" + encodeURIComponent($("#dialog-editsetpoint #devicename").val()) + "&description=" + encodeURIComponent($("#dialog-editsetpoint #devicedescription").val()) + "&setpoint=" + setpoint + "&mode=" + (tUntil != "" ? "TemporaryOverride" : "PermanentOverride") + "&until=" + tUntil + "&used=true",
                        async: false,
                        dataType: "json",
                        success: function(data) {
                            ShowTemps();
            ...
            dialog_editsetpoint_buttons[$.t("Cancel Override")] = function() {
                ...
                if (bValid) {
                    ...
                    if (aValue > 35) aValue = 35;
                    $.ajax({
                        url: "json.htm?type=setused&idx=" + $.devIdx + "&name=" + encodeURIComponent($("#dialog-editsetpoint #devicename").val()) + "&description=" + encodeURIComponent($("#dialog-editsetpoint #devicedescription").val()) + "&setpoint=" + aValue + "&mode=Auto&used=true",
                        async: false,
                        dataType: "json",
                        success: function(data) {
                            ShowTemps();
            ...
            dialog_editstate_buttons[$.t("Set")] = function() {
                ...
                if (bValid) {
                    ...
                    var tUntil = "";
                    if ($("#dialog-editstate #edittable #until_state").val() != "") tUntil = $("#dialog-editstate #edittable #until_state").datetimepicker("getDate").toISOString();
                    $.ajax({
                        url: "json.htm?type=setused&idx=" + $.devIdx + "&name=" + encodeURIComponent($("#dialog-editstate #devicename").val()) + "&description=" + encodeURIComponent($("#dialog-editstate #devicedescription").val()) + "&state=" + aValue + "&mode=" + (tUntil != "" ? "TemporaryOverride" : "PermanentOverride") + "&until=" + tUntil + "&used=true",
                        async: false,
                        dataType: "json",
                        success: function(data) {
                            ShowTemps();
            ...
            dialog_edittempdevicesmall_buttons[$.t("Update")] = function() {
                ...
                if (bValid) {
                    $(this).dialog("close");
                    $.ajax({
                        url: "json.htm?type=setused&idx=" + $.devIdx + "&name=" + encodeURIComponent($("#dialog-edittempdevicesmall #devicename").val()) + "&description=" + encodeURIComponent($("#dialog-edittempdevicesmall #devicedescription").val()) + "&used=true",
                        async: false,
                        dataType: "json",
                        success: function(data) {
                            ShowTemps();
            ...
            controller: function($scope, $element, $attrs, permissions) {
                ...
                ctrl.dtUntil = function() {
                    if (angular.isDefined(item.Until)) {
                        ...
                        dtUntil = new Date(dtUntil.getTime() - dtUntil.getTimezoneOffset() * 6e4);
