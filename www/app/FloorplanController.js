    ...
    app.controller("FloorplanController", [ "$scope", "$rootScope", "$location", "$window", "$http", "$interval", "$timeout", "$compile", "permissions", function($scope, $rootScope, $location, $window, $http, $interval, $timeout, $compile, permissions) {
        ...
        function FPtouchend(e) {
            if (e.target.getAttribute("related") != null) {
                $("#BulletImages").children().css({
                    display: "none"
                });
                if ($scope.debug > 0) $.cachenoty = generate_noty("info", "<b>Scrolling to: " + e.target.getAttribute("related") + "</b>", 1e3);
                e.preventDefault();
                ...
                if ($scope.isScrolling == true) {
                    ...
                    $timeout(function() {
                        if ($scope.isScrolling == false && $scope.pendingScroll == true) {
                            if ($scope.debug > 0) $.cachenoty = generate_noty("info", "<b>Scrolled to: " + window.pageXOffset + "</b>", 1e3);
                            $scope.pendingScroll = false;
                            ...
                            if ($scope.debug > 0) $.cachenoty = generate_noty("info", "<b>Closest is: " + nearestFP.attr("id") + "</b>", 1e3);
                    ...
                    var delay = 500;
                    if ($scope.debug > 0) $.cachenoty = generate_noty("info", "<b>Tap Delta: " + delta + "</b>", 1e3);
        ...
        ScrollFloorplans = function(tagName, animate) {
            if ($scope.debug > 0) $.cachenoty = generate_noty("info", "<b>Scrolling to: " + tagName + "</b>", 1e3);
                ...
                for (var i = 0, len = $scope.floorPlans.length; i < len; i++) {
                    if ($scope.floorPlans[i].idx == target.getAttribute("index")) {
                        if (allowAnimation == false) {
                            ...
                            var to = {
                                property: $("#floorplancontent").width() * i
                            };
            ...
            if (target != null) {
                for (var i = 0, len = $scope.floorPlans.length; i < len; i++) {
                    if ($scope.floorPlans[i].idx == target.parentNode.getAttribute("index")) {
                        ...
                        svgCont.appendChild(makeSVGnode("g", {
                            id: tagName + "_grp",
                            transform: "translate(0,0) scale(1)",
                            style: "",
                            zoomed: "false"
                        }, ""));
                        svgCont.childNodes[0].appendChild(makeSVGnode("image", {
                            width: "100%",
                            height: "100%",
                            "xlink:href": $scope.floorPlans[i].Image
                        }, ""));
                        svgCont.childNodes[0].appendChild(makeSVGnode("g", {
                            id: tagName + "_Content",
                            "class": "FloorContent"
                        }, ""));
                        svgCont.childNodes[0].appendChild(makeSVGnode("g", {
                ...
                if (AllLoaded == true) {
                    Device.checkDefs();
                    $(".FloorRooms").click(function(event) {
                        $scope.RoomClick(event);
                    });
                    $scope.mytimer = $interval(function() {
                        RefreshFPDevices(false);
                    }, 1e4);
        ...
        $scope.FloorplanResize = function() {
            if (typeof $("#floorplancontent") != "undefined") {
                ...
                if ($scope.debug > 0) $.cachenoty = generate_noty("info", "<b>Window: " + $window.innerWidth + "x" + $window.innerHeight + "</b><br/><b>View: " + $("#floorplancontent").width() + "x" + wrpHeight + "</b>", 1e4);
                $(".imageparent").each(function(i) {
                    $("#" + $(this).attr("id") + "_svg").width($("#floorplancontent").width()).height(wrpHeight);
                });
                if ($scope.FloorplanCount > 1) {
                    $("#BulletGroup:first").css("left", ($window.innerWidth - $("#BulletGroup:first").width()) / 2).css("bottom", $("#copyright").css("display") == "none" ? 10 : $("#copyright").height() + 10).css("display", "inline");
                }
        ...
        $scope.doubleClick = function() {
            if ($.myglobals.FullscreenMode == true) {
                if ($("#fpwrapper").attr("fullscreen") != "true") {
                    if ($scope.debug > 1) $.cachenoty = generate_noty("info", "<b>Double Click -> Fullscreen!!</b>", 1e3);
                    $("#copyright").css({
                        display: "none"
                    });
                    $(".navbar").css({
                ...
                $.cachenoty = generate_noty("warning", "<b>" + $.t("Fullscreen mode is disabled") + "</b>", 3e3);
            }
        };
        $scope.RoomClick = function(click) {
            $(".DeviceDetails").css("display", "none");
            var borderRect = click.target.getBBox();
            var margin = .1;
            ...
            if ($("#" + svgFloor).attr("zoomed") == "true") {
                if ($.myglobals.AnimateTransitions) {
                    $("#" + svgFloor).css("paddingTop", borderRect.y - marginY).css("paddingLeft", borderRect.x - marginX).css("paddingRight", scale).animate({
                        paddingTop: 0,
                        paddingLeft: 0,
                        paddingRight: 1
                    }, {
                        ...
                        progress: function(animation, progress, remainingMs) {
                            $scope.animateMove(svgFloor);
                        },
                        always: function() {
                            $("#" + svgFloor).attr("zoomed", "false").css("paddingTop", "").css("paddingLeft", "").css("paddingRight", "").attr("transform", "translate(0,0) scale(1)");
                        }
                    });
                } else {
                    $("#" + svgFloor).attr("zoomed", "false").css("paddingTop", "").css("paddingLeft", "").css("paddingRight", "").attr("transform", "translate(0,0) scale(1)");
                ...
                if ($.myglobals.AnimateTransitions) {
                    $("#" + svgFloor).css("paddingTop", 0).css("paddingLeft", 0).css("paddingRight", 1).animate({
                        ...
                        always: function() {
                            $("#" + svgFloor).attr("zoomed", "true").css("paddingTop", "").css("paddingLeft", "").css("paddingRight", "").attr("transform", attr);
                        }
                    });
                } else {
                        ...
                        $(".Device_" + item.idx).each(function() {
                            ...
                            try {
                                ...
                                if (existing != undefined) {
                                    if ($scope.debug > 2) $.cachenoty = generate_noty("info", "<b>Refreshing Device " + dev.name + (compoundDevice ? " - " + item.Type : "") + "</b>", 2e3);
                                    dev.htmlMinimum(existing.parentNode);
                                }
                            } catch (err) {
                                $.cachenoty = generate_noty("error", "<b>Device refresh error</b> " + dev.name + "<br>" + err, 5e3);
                            }
                        });
                    });
                }
                if (bOneOff != true) $scope.mytimer = $interval(function() {
                    RefreshFPDevices(false);
                }, 1e4);
            }).error(function() {
                if (bOneOff != true) $scope.mytimer = $interval(function() {
        ...
        $scope.ShowFPDevices = function(floorIdx) {
            $http({
                ...
                if (typeof data.result != "undefined") {
                    ...
                    var elIcons = makeSVGnode("g", {
                        id: "DeviceIcons"
                    }, "");
                    elDevices.appendChild(elIcons);
                    var elDetails = makeSVGnode("g", {
                        id: "DeviceDetails"
                    }, "");
        ...
        $scope.ShowRooms = function(floorIdx) {
            $http({
                ...
                if (typeof data.result != "undefined") {
                    $.each(data.result, function(i, item) {
                        $("#" + $scope.floorPlans[floorIdx].tagName + "_Rooms").append(makeSVGnode("polygon", {
                            id: item.Name,
                            "class": "hoverable",
                            points: item.Area
                        }, item.Name));
                    });
                    if (typeof $.myglobals.RoomColour != "undefined" && typeof $.myglobals.InactiveRoomOpacity != "undefined") {
                        $(".hoverable").css({
        ...
        ShowFloorplans = function(floorIdx) {
            ...
            $http({
                ...
                if (typeof data.result != "undefined") {
                    if (typeof $scope.actFloorplan == "undefined") {
                        ...
                        if ($scope.debug > 0) $.cachenoty = generate_noty("info", "<b>Floorplan already set to: " + $scope.actFloorplan + "</b>", 5e3);
                    }
                    ...
                    $.each(data.result, function(i, item) {
                        ...
                        var thisFP = $scope.makeHTMLnode("div", {
                            id: $scope.floorPlans[i].tagName,
                            index: item.idx,
                            order: item.Order,
                        ...
                        thisFP.appendChild($scope.makeHTMLnode("img", {
                            id: $scope.floorPlans[i].tagName + "_img",
                            src: item.Image,
                            align: "top",
                            onload: "ImgLoaded('" + $scope.floorPlans[i].tagName + "');",
                            "class": "floorplan",
                            style: "display:none"
                        }));
                        thisFP.appendChild(makeSVGnode("svg", {
                            id: tagName + "_svg",
                            width: "100%",
                            height: "100%",
                            version: "1.1",
                        ...
                        if ($scope.FloorplanCount > 1) {
                            var bulletTd = $scope.makeHTMLnode("td", {
                                "class": "bulletcell",
                                width: 100 / $scope.FloorplanCount + "%"
                            });
                            $("#BulletRow:first").append(bulletTd);
                            bulletTd.appendChild($scope.makeHTMLnode("div", {
                                "class": "bullet",
                                title: item.Name,
                                related: $scope.floorPlans[i].tagName
                            }));
                            $("#BulletImages").append($scope.makeHTMLnode("img", {
                                id: $scope.floorPlans[i].tagName + "_bullet",
                                src: item.Image,
                                related: $scope.floorPlans[i].tagName
                            }));
                        }
                ...
                $(".bulletcell").hover(function() {
                    $(this).children().css({
                        background: $.myglobals.RoomColour
                    });
                    $("#" + $(this).children().attr("related") + "_bullet").css({
                        display: "inline"
                    });
                }, function() {
                    $(this).children().css({
            ...
            $("body").css("overflow", "hidden").on("pageexit", function() {
                ...
                if ($scope.debug > 0) $.cachenoty = generate_noty("info", "<b>PageExit code executed</b>", 2e3);
            });
        };
        init();
        function init() {
