    ...
    app.controller("SetupController", [ "$scope", "$rootScope", "$window", "$location", "$http", "$interval", function($scope, $rootScope, $window, $location, $http, $interval) {
        ...
        $scope.TestNotification = function(subsystem) {
            ...
            switch (subsystem) {
              ...
              case "email":
                ...
                if (EmailServer == "") {
                    ShowNotify($.t("Invalid Email Settings..."), 2e3, true);
                    return;
                }
                ...
                if (EmailFrom == "" || EmailTo == "") {
                    ShowNotify($.t("Invalid Email From/To Settings..."), 2e3, true);
                    return;
                }
                if (EmailUsername != "" && EmailPassword == "") {
                    ShowNotify($.t("Please enter an Email Password..."), 2e3, true);
                    return;
                }
                ...
                success: function(data) {
                    if (data.status != "OK") {
                        ...
                        if (subsystem == "http" || subsystem == "kodi" || subsystem == "lms") {
                            ShowNotify($.t("Problem Sending Notification"), 3e3, true);
                        } else if (subsystem == "email") {
                            ShowNotify($.t("Problem sending Email, please check credentials!"), 3e3, true);
                        } else {
                            ShowNotify($.t("Problem sending notification, please check the API key!"), 3e3, true);
                        }
                        return;
                    } else {
                        HideNotify();
                        ShowNotify($.t("Notification sent!<br>Should arrive at your device soon..."), 3e3);
                    }
                },
                error: function() {
                    HideNotify();
                    if (subsystem == "email") {
                        ShowNotify($.t("Invalid Email Settings..."), 3e3, true);
                    } else {
                        ShowNotify($.t("Problem sending notification, please check the API key!"), 3e3, true);
                    }
                }
        ...
        $scope.StoreSettings = function(formname) {
            ...
            if (Latitude == "" || Longitude == "" || (isNaN(Latitude) || isNaN(Longitude))) {
                ShowNotify($.t("Invalid Location Settings..."), 2e3, true);
                return;
            }
            ...
            if (EmailServer != "") {
                ...
                if (EmailPort == "") {
                    ShowNotify($.t("Please enter an Email Port..."), 2e3, true);
                    return;
                }
            ...
            if (popupDelay != "") {
                if (!$.isNumeric(popupDelay)) {
                    ShowNotify($.t("Popup Delay can only contain numbers..."), 2e3, true);
                    return;
                }
            }
