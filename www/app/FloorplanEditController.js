    ...
    app.controller("FloorplanEditController", [ "$scope", "$rootScope", "$location", "$http", "$interval", "permissions", function($scope, $rootScope, $location, $http, $interval, permissions) {
            ...
            if ($("#floorplangroup")[0].getAttribute("zoomed") != "true") {
                if ($("#roompolyarea").attr("title") != "") {
                    ...
                    if (points != "") {
                        ...
                        $("#floorplangroup")[0].appendChild(makeSVGnode("circle", {
                            id: "firstclick",
                            cx: xPoint,
                            cy: yPoint,
                            r: 2,
        ...
        GetFloorplanSettings = function() {
            ...
            if (!$.isNumeric(csettings.scalefactor)) {
                ShowNotify($.t("Icon Scale can only contain numbers..."), 2e3, true);
                return;
            }
            return csettings;
        };
        ...
        AddUnusedPlan = function() {
            ...
            $.ajax({
                url: "json.htm?type=command&param=addfloorplanplan&idx=" + $.devIdx + "&planidx=" + PlanIdx,
                async: false,
                dataType: "json",
                success: function(data) {
                    if (data.status == "OK") {
        ...
        UpdateFloorplanPlan = function(planidx, clear) {
            ...
            $.ajax({
                url: "json.htm?type=command&param=updatefloorplanplan&planidx=" + planidx + "&area=" + PlanArea,
                async: false,
                dataType: "json",
                success: function(data) {
            ...
            for (var i = 0; i < deviceParent.childNodes.length; i++) {
                if (deviceParent.childNodes[i].getAttribute("onfloorplan") != "false") {
                    $.ajax({
                        url: "json.htm?type=command&param=setplandevicecoords" + "&idx=" + deviceParent.childNodes[i].getAttribute("idx") + "&planidx=" + planidx + "&xoffset=" + deviceParent.childNodes[i].getAttribute("xoffset") + "&yoffset=" + deviceParent.childNodes[i].getAttribute("yoffset") + "&DevSceneType=" + deviceParent.childNodes[i].getAttribute("devscenetype"),
                        async: false,
                        dataType: "json",
                        success: function(data) {
                            if (data.status == "OK") {} else {
        ...
        ConfirmNoUpdate = function(param, yesFunc) {
            if ($("#floorplaneditcontent #delclractive #activeplanupdate").attr("class") == "btnstyle3") {
                bootbox.confirm($.t("You have unsaved changes, do you want to continue?"), function(result) {
                    if (result == true) {
                        yesFunc(param);
                    }
                });
        ...
        RefreshDevices = function() {
            if (typeof $("#floorplangroup") != "undefined" && typeof $("#floorplangroup")[0] != "undefined") {
                if (typeof $scope.mytimer != "undefined") {
                    $interval.cancel($scope.mytimer);
                    $scope.mytimer = undefined;
                }
        ...
        function init() {
            ...
            ShowFloorplans();
            $("#floorplanimage").on("click", function(event) {
                FloorplanClick(event);
            });
            $("#guidelines").on("click", function(event) {
                FloorplanClick(event);
            });
            $("#roompolyarea").on("click", function(event) {
                PolyClick(event);
            });
            $("#svgcontainer").on("mouseleave", function(event) {
                MouseOut(event);
            });
