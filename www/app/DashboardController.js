define([ "app" ], function(app) {
    app.controller("DashboardController", [ "$scope", "$rootScope", "$location", "$http", "$interval", "$window", "permissions", function($scope, $rootScope, $location, $http, $interval, $window, permissions) {
        ...
        EvoDisplayTextMode = function(strstatus) {
            if (strstatus == "Auto") strstatus = "Normal"; else if (strstatus == "AutoWithEco") strstatus = "Economy"; else if (strstatus == "DayOff") strstatus = "Day Off"; else if (strstatus == "HeatingOff") strstatus = "Heating Off";
            return strstatus;
        };
        GetLightStatusText = function(item) {
            if (item.SubType == "Evohome") return EvoDisplayTextMode(item.Status); else if (item.SwitchType === "Selector") return item.LevelNames.split("|")[item.LevelInt / 10]; else return item.Status;
        };
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (typeof data.result != "undefined") {
                                ...
                                if (typeof obj != "undefined") {
                                    if ($scope.config.DashboardType == 2 || window.myglobals.ismobile == true) {
                                        ...
                                        if (item.Type.indexOf("Group") == 0) {
                                            if (item.Status == "Off") {
                                                status += '<button class="btn btn-mini" type="button" onclick="SwitchScene(' + item.idx + ",'On',RefreshFavorites, " + item.Protected + ');">' + $.t("On") + "</button> " + '<button class="btn btn-mini btn-info" type="button" onclick="SwitchScene(' + item.idx + ",'Off',RefreshFavorites, " + item.Protected + ');">' + $.t("Off") + "</button>";
                                            } else {
                                                status += '<button class="btn btn-mini btn-info" type="button" onclick="SwitchScene(' + item.idx + ",'On',RefreshFavorites, " + item.Protected + ');">' + $.t("On") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchScene(' + item.idx + ",'Off',RefreshFavorites, " + item.Protected + ');">' + $.t("Off") + "</button>";
                                            }
                        ...
                        $.each(data.result, function(i, item) {
                            var isdimmer = false;
                            if ((item.Type.indexOf("Light") == 0 || item.Type.indexOf("Blind") == 0 || item.Type.indexOf("Curtain") == 0 || item.Type.indexOf("Thermostat 2") == 0 || item.Type.indexOf("Thermostat 3") == 0 || item.Type.indexOf("Chime") == 0 || item.Type.indexOf("RFY") == 0 || item.Type.indexOf("ASA") == 0 || item.SubType == "Smartwares Mode" || item.SubType == "Relay" || typeof item.SubType != "undefined" && item.SubType.indexOf("Itho") == 0 || typeof item.SubType != "undefined" && item.SubType.indexOf("Lucci") == 0) && item.Favorite != 0) {
                                    ...
                                    if ($scope.config.DashboardType == 2 || window.myglobals.ismobile == true) {
                                        ...
                                        if (item.SwitchType == "Doorbell") {
                                            ...
                                            if (item.InternalState == "Open") {
                                                status = '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Lock") + "</button>";
                                            } else {
                                                status = '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Locked") + "</button>";
                                            }
                                        } else if (item.SwitchType == "Door Lock") {
                                            if (item.InternalState == "Unlocked") {
                                                status = '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Unlocked") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Locked") + "</button>";
                                            } else {
                                                status = '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Unlocked") + "</button> " + '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Locked") + "</button>";
                                            }
                                            ...
                                            if (item.Status == "On" || item.Status == "Chime" || item.Status == "Group On" || item.Status == "All On") {
                                            ...
                                            if (item.SubType == "RAEX" || item.SubType.indexOf("A-OK") == 0 || item.SubType.indexOf("Hasta") >= 0 || item.SubType.indexOf("Media Mount") == 0 || item.SubType.indexOf("Forest") == 0 || item.SubType.indexOf("Chamberlain") == 0 || item.SubType.indexOf("Sunpery") == 0 || item.SubType.indexOf("Dolat") == 0 || item.SubType.indexOf("ASP") == 0 || item.SubType == "Harrison" || item.SubType.indexOf("RFY") == 0 || item.SubType.indexOf("ASA") == 0 || item.SubType.indexOf("DC106") == 0 || item.SubType.indexOf("Confexx") == 0 || item.SwitchType.indexOf("Venetian Blinds") == 0) {
                                                if (item.Status == "Closed") {
                                                    status = '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini btn-danger" type="button" onclick="SwitchLight(' + item.idx + ",'Stop',RefreshFavorites," + item.Protected + ');">' + $.t("Stop") + "</button> " + '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Closed") + "</button>";
                                                } else {
                                                    status = '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini btn-danger" type="button" onclick="SwitchLight(' + item.idx + ",'Stop',RefreshFavorites," + item.Protected + ');">' + $.t("Stop") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Close") + "</button>";
                                                }
                                            } else {
                                                if (item.Status == "Closed") {
                                                    status = '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Closed") + "</button>";
                                                } else {
                                                    status = '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Close") + "</button>";
                                                }
                                            }
                                        } else if (item.SwitchType == "Blinds Inverted") {
                                            if (item.SubType == "RAEX" || item.SubType.indexOf("A-OK") == 0 || item.SubType.indexOf("Hasta") >= 0 || item.SubType.indexOf("Media Mount") == 0 || item.SubType.indexOf("Forest") == 0 || item.SubType.indexOf("Chamberlain") == 0 || item.SubType.indexOf("Sunpery") == 0 || item.SubType.indexOf("Dolat") == 0 || item.SubType.indexOf("ASP") == 0 || item.SubType == "Harrison" || item.SubType.indexOf("RFY") == 0 || item.SubType.indexOf("ASA") == 0 || item.SubType.indexOf("DC106") == 0 || item.SubType.indexOf("Confexx") == 0) {
                                                if (item.Status == "Closed") {
                                                    status = '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini btn-danger" type="button" onclick="SwitchLight(' + item.idx + ",'Stop',RefreshFavorites," + item.Protected + ');">' + $.t("Stop") + "</button> " + '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Closed") + "</button>";
                                                } else {
                                                    status = '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini btn-danger" type="button" onclick="SwitchLight(' + item.idx + ",'Stop',RefreshFavorites," + item.Protected + ');">' + $.t("Stop") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Close") + "</button>";
                                                }
                                            } else {
                                                if (item.Status == "Closed") {
                                                    status = '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Closed") + "</button>";
                                                } else {
                                                    status = '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites)," + item.Protected + ';">' + $.t("Close") + "</button>";
                                                }
                                            }
                                            ...
                                            isdimmer = true;
                                            var img = "";
                                            if (item.Status == "On" || item.Status == "Chime" || item.Status == "Group On" || item.Status.indexOf("Set ") == 0) {
                                                status = '<label id="statustext"><button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("On") + "</button></label> " + '<label id="img"><button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Off") + "</button></label>";
                                            } else {
                                                status = '<label id="statustext"><button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("On") + "</button></label> " + '<label id="img"><button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Off") + "</button></label>";
                                            }
                                            ...
                                            var img = "";
                                            if (item.Status == "On") {
                                                status = '<label id="statustext"><button class="btn btn-mini btn-info" type="button" ' + (RO ? "disabled" : "") + ' onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("On") + "</button></label> " + '<label id="img"><button class="btn btn-mini" type="button" ' + (RO ? "disabled" : "") + ' onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Off") + "</button></label>";
                                            } else {
                                                status = '<label id="statustext"><button class="btn btn-mini" type="button" ' + (RO ? "disabled" : "") + ' onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("On") + "</button></label> " + '<label id="img"><button class="btn btn-mini btn-info" type="button" ' + (RO ? "disabled" : "") + ' onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Off") + "</button></label>";
                                            }
                                            ...
                                            if (typeof selector$ !== "undefined") {
                                                if (item.SelectorStyle === 0) {
                                                    selector$.find("label").removeClass("ui-state-active").removeClass("ui-state-focus").removeClass("ui-state-hover").end().find("input:radio").removeProp("checked").filter('[value="' + item.LevelInt + '"]').prop("checked", true).end().end().buttonset("refresh");
                                            ...
                                            status = '<button class="' + class_1 + '" type="button" onclick="SwitchLight(' + item.idx + ",'1',RefreshFavorites," + item.Protected + ');">' + $.t("1") + "</button> " + '<button class="' + class_2 + '" type="button" onclick="SwitchLight(' + item.idx + ",'2',RefreshFavorites," + item.Protected + ');">' + $.t("2") + "</button> " + '<button class="' + class_3 + '" type="button" onclick="SwitchLight(' + item.idx + ",'3',RefreshFavorites," + item.Protected + ');">' + $.t("3") + "</button> " + '<button class="' + class_timer + '" type="button" onclick="SwitchLight(' + item.idx + ",'timer',RefreshFavorites," + item.Protected + ');">' + $.t("Timer") + "</button>";
                                            ...
                                            status = '<button class="' + class_1 + '" type="button" onclick="SwitchLight(' + item.idx + ",'hi',RefreshFavorites," + item.Protected + ');">' + $.t("hi") + "</button> " + '<button class="' + class_2 + '" type="button" onclick="SwitchLight(' + item.idx + ",'med',RefreshFavorites," + item.Protected + ');">' + $.t("med") + "</button> " + '<button class="' + class_3 + '" type="button" onclick="SwitchLight(' + item.idx + ",'low',RefreshFavorites," + item.Protected + ');">' + $.t("low") + "</button> " + '<button class="' + class_4 + '" type="button" onclick="SwitchLight(' + item.idx + ",'off',RefreshFavorites," + item.Protected + ');">' + $.t("Off") + "</button> " + '<button class="' + class_light + '" type="button" onclick="SwitchLight(' + item.idx + ",'light',RefreshFavorites," + item.Protected + ');">' + $.t("Light") + "</button>";
                                        } else {
                                            if (item.Status == "On" || item.Status == "Chime" || item.Status == "Group On" || item.Status.indexOf("Down") != -1 || item.Status.indexOf("Up") != -1 || item.Status.indexOf("Set ") == 0) {
                                                status = '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("On") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Off") + "</button>";
                                            } else {
                                                status = '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("On") + "</button> " + '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Off") + "</button>";
                                            }
                                        }
                                        if (isdimmer == true) {
                                        ...
                                        if (item.SwitchType == "Doorbell") {
                                            ...
                                            if (item.Status == "On" || item.Status == "Chime" || item.Status.indexOf("Down") != -1 || item.Status.indexOf("Up") != -1 || item.Status == "Group On" || item.Status.indexOf("Set ") == 0) {
                                        ...
                                        if (item.SwitchType === "Selector") {
                                            ...
                                            if (typeof selector$ !== "undefined") {
                                                if (item.SelectorStyle === 0) {
                                                    selector$.find("label").removeClass("ui-state-active").removeClass("ui-state-focus").end().find("input:radio").removeProp("checked").filter('[value="' + item.LevelInt + '"]').prop("checked", true).end().end().buttonset("refresh");
                                                } else if (item.SelectorStyle === 1) {
                                                    selector$.val(item.LevelInt);
                                                    selector$.selectmenu("refresh");
                                                }
                        ...
                        $.each(data.result, function(i, item) {
                            if ((typeof item.Temp != "undefined" || typeof item.Humidity != "undefined" || typeof item.Chill != "undefined") && item.Favorite != 0) {
                        ...
                        $.each(data.result, function(i, item) {
                            if ((typeof item.Rain != "undefined" || typeof item.Visibility != "undefined" || typeof item.UVI != "undefined" || typeof item.Radiation != "undefined" || typeof item.Direction != "undefined" || typeof item.Barometer != "undefined") && item.Favorite != 0) {
                        ...
                        $.each(data.result, function(i, item) {
                            if ((typeof item.Counter != "undefined" || item.Type == "Current" || item.Type == "Energy" || item.Type == "Current/Energy" || item.Type == "Power" || item.Type == "Air Quality" || item.Type == "Lux" || item.Type == "Weight" || item.Type == "Usage" || item.SubType == "Percentage" || item.Type == "Thermostat" && item.SubType == "SetPoint" || item.SubType == "kWh" || item.SubType == "Soil Moisture" || item.SubType == "Leaf Wetness" || item.SubType == "Voltage" || item.SubType == "Distance" || item.SubType == "Current" || item.SubType == "Text" || item.SubType == "Alert" || item.SubType == "Pressure" || item.SubType == "A/D" || item.SubType == "Thermostat Mode" || item.SubType == "Thermostat Fan Mode" || item.SubType == "Smartwares" || item.SubType == "Waterflow" || item.SubType == "Sound Level" || item.SubType == "Custom Sensor") && item.Favorite != 0) {
                                ...
                                if (typeof obj != "undefined") {
                                    if ($scope.config.DashboardType == 2 || window.myglobals.ismobile == true) {
                                        ...
                                        if (typeof item.Counter != "undefined") {
                                            ...
                                            status += item.Data;
                                        } else if (item.Type == "Energy" || item.Type == "Current/Energy" || item.Type == "Power" || item.SubType == "kWh" || item.Type == "Air Quality" || item.Type == "Lux" || item.Type == "Weight" || item.Type == "Usage" || item.SubType == "Percentage" || item.Type == "Fan" || item.SubType == "Soil Moisture" || item.SubType == "Leaf Wetness" || item.SubType == "Voltage" || item.SubType == "Distance" || item.SubType == "Current" || item.SubType == "Text" || item.SubType == "Pressure" || item.SubType == "A/D" || item.SubType == "Waterflow" || item.SubType == "Sound Level" || item.SubType == "Custom Sensor") {
        ...
        ShowFavorites = function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (typeof data.result != "undefined") {
                        ...
                        bHaveAddedDevider = false;
                        $.each(data.result, function(i, item) {
                            if (item.Favorite != 0 && (item.Type.indexOf("Light") == 0 || item.SubType == "Smartwares Mode" || item.Type.indexOf("Blind") == 0 || item.Type.indexOf("Curtain") == 0 || item.Type.indexOf("Thermostat 2") == 0 || item.Type.indexOf("Thermostat 3") == 0 || item.Type.indexOf("Chime") == 0 || item.Type.indexOf("RFY") == 0 || item.Type.indexOf("ASA") == 0 || item.SubType == "Relay" || typeof item.SubType != "undefined" && item.SubType.indexOf("Itho") == 0 || typeof item.SubType != "undefined" && item.SubType.indexOf("Lucci") == 0 || item.Type.indexOf("Value") == 0 && typeof item.SwitchType != "undefined")) {
                                totdevices += 1;
                                if (jj == 0) {
                                    if ($scope.config.DashboardType == 2 || window.myglobals.ismobile == true) {
                                        if (htmlcontent != "") {
                                ...
                                if ($scope.config.DashboardType == 2 || window.myglobals.ismobile == true) {
                                    ...
                                    if (item.SwitchType == "Doorbell") {
                                        ...
                                        if (item.InternalState == "Open") {
                                            status = '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Lock") + "</button>";
                                        } else {
                                            status = '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Open") + "</button> " + '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Locked") + "</button>";
                                        }
                                    } else if (item.SwitchType == "Door Lock") {
                                        if (item.InternalState == "Unlocked") {
                                            status = '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Unlocked") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("Locked") + "</button>";
                                        ...
                                        status = '<button class="' + class_1 + '" type="button" onclick="SwitchLight(' + item.idx + ",'hi',RefreshFavorites," + item.Protected + ');">' + $.t("hi") + "</button> " + '<button class="' + class_2 + '" type="button" onclick="SwitchLight(' + item.idx + ",'med',RefreshFavorites," + item.Protected + ');">' + $.t("med") + "</button> " + '<button class="' + class_3 + '" type="button" onclick="SwitchLight(' + item.idx + ",'low',RefreshFavorites," + item.Protected + ');">' + $.t("low") + "</button> " + '<button class="' + class_4 + '" type="button" onclick="SwitchLight(' + item.idx + ",'off',RefreshFavorites," + item.Protected + ');">' + $.t("Off") + "</button> " + '<button class="' + class_timer + '" type="button" onclick="SwitchLight(' + item.idx + ",'light',RefreshFavorites," + item.Protected + ');">' + $.t("Light") + "</button>";
                                    } else {
                                        if (item.Status == "On" || item.Status == "Chime" || item.Status == "Group On" || item.Status.indexOf("Down") != -1 || item.Status.indexOf("Up") != -1 || item.Status.indexOf("Set ") == 0) {
                                            status = '<button class="btn btn-mini btn-info" type="button" onclick="SwitchLight(' + item.idx + ",'On',RefreshFavorites," + item.Protected + ');">' + $.t("On") + "</button> " + '<button class="btn btn-mini" type="button" onclick="SwitchLight(' + item.idx + ",'Off',RefreshFavorites," + item.Protected + ');">' + $.t("Off") + "</button>";
                                        } else {
                                    ...
                                    if (item.SwitchType == "Dimmer") {
                                        ...
                                        if (item.Unit < 64 || item.Unit > 95) xhtm += ' data-disabled="true"';
                                        xhtm += "></div>";
                                        xhtm += "</td>";
                                        xhtm += "</tr>";
                                    ...
                                    if (item.SwitchType == "Doorbell") {
                                        ...
                                        if (item.CustomImage == 0) item.Image = item.TypeImg;
                                        item.Image = item.Image.charAt(0).toUpperCase() + item.Image.slice(1);
                                        if (item.Status == "On" || item.Status == "Chime" || item.Status == "Group On" || item.Status.indexOf("Set ") == 0 || item.Status.indexOf("NightMode") == 0 || item.Status.indexOf("Disco ") == 0) {
                                            if (item.SubType == "RGB") {
                            ...
                            if (item.Type.indexOf("Heating") == 0 && item.Favorite != 0) {
                                ...
                                if ($scope.config.DashboardType == 2 || window.myglobals.ismobile == true) {
                                    ...
                                    if (item.SubType == "Evohome") {
                                        ...
                                        xhtm += '	      <td id="status">' + TranslateStatus(EvoDisplayTextMode(item.Status)) + "</td>\n" + '	      <td id="lastupdate">' + item.LastUpdate + "</td>\n" + "	    </tr>\n" + "	    </table>\n" + "	  </section>\n" + "	</div>\n";
                                    }
                                }
                                htmlcontent += xhtm;
                                jj += 1;
                        ...
                        bHaveAddedDevider = false;
                        $.each(data.result, function(i, item) {
                            if ((typeof item.Counter != "undefined" || item.Type == "Current" || item.Type == "Energy" || item.SubType == "kWh" || item.Type == "Current/Energy" || item.Type == "Power" || item.Type == "Air Quality" || item.Type == "Lux" || item.Type == "Weight" || item.Type == "Usage" || item.SubType == "Percentage" || item.Type == "Thermostat" && item.SubType == "SetPoint" || item.SubType == "Soil Moisture" || item.SubType == "Leaf Wetness" || item.SubType == "Voltage" || item.SubType == "Distance" || item.SubType == "Current" || item.SubType == "Text" || item.SubType == "Alert" || item.SubType == "Pressure" || item.SubType == "A/D" || item.SubType == "Thermostat Mode" || item.SubType == "Thermostat Fan Mode" || item.SubType == "Smartwares" || item.SubType == "Waterflow" || item.SubType == "Sound Level" || item.SubType == "Custom Sensor") && item.Favorite != 0) {
                                totdevices += 1;
                                if (jj == 0) {
                                    if ($scope.config.DashboardType == 2 || window.myglobals.ismobile == true) {
                                        if (htmlcontent != "") {
                                ...
                                if ($scope.config.DashboardType == 2 || window.myglobals.ismobile == true) {
                                    ...
                                    } else if (item.Type == "Air Quality" || item.Type == "Lux" || item.Type == "Weight" || item.Type == "Usage" || item.SubType == "Percentage" || item.Type == "Fan" || item.SubType == "Soil Moisture" || item.SubType == "Leaf Wetness" || item.SubType == "Voltage" || item.SubType == "Distance" || item.SubType == "Current" || item.SubType == "Pressure" || item.SubType == "A/D" || item.SubType == "Sound Level" || item.SubType == "Waterflow" || item.Type == "Current" || item.SubType == "Custom Sensor") {
                                        xhtm += item.Data;
                                    } else if (item.Type == "Thermostat" && item.SubType == "SetPoint") {
            ...
            if (htmlcontent == "") {
                htmlcontent = "<h2>" + $.t("No favorite devices defined ... (Or communication Lost!)") + "</h2><br>\n" + $.t("If this is your first time here, please setup your") + ' <a href="javascript:SwitchLayout(\'Hardware\')" data-i18n="Hardware">Hardware</a>, ' + $.t("and add some") + ' <a href="javascript:SwitchLayout(\'Devices\')" data-i18n="Devices">Devices</a>.';
            } else {
                htmlcontent += "<br>";
            }
            var suntext = "";
            if (bShowRoomplan == false) {
                suntext = '	<table border="0" cellpadding="0" cellspacing="0" width="100%">\n' + "	<tr>\n" + '	  <td align="left" valign="top" id="timesun"></td>\n' + "	</tr>\n" + "	</table>\n";
            } else {
                suntext = "<div>" + '<table border="0" cellpadding="0" cellspacing="0" width="100%">' + "<tr>" + '<td align="left" valign="top" id="timesun"></td>' + '<td align="right">' + '<span data-i18n="Room">Room</span>:&nbsp;<select id="comboroom" style="width:160px" class="combobox ui-corner-all">' + '<option value="0" data-i18n="All">All</option>' + "</select>" + "</td>" + "</tr>" + "</table>" + "</div><br>";
            }
            ...
            var accordion_head = $("#dashcontent .accordion > li > a"), accordion_body = $("#dashcontent .accordion li > .sub-menu");
            ...
            $("#dashcontent .dimslider").slider({
                ...
                create: function(event, ui) {
                    ...
                    if ($(this).data("disabled")) $(this).slider("option", "disabled", true);
                },
                slide: function(event, ui) {
                    ...
                    var fPercentage = parseInt(100 / (maxValue - 1) * (ui.value - 1));
                    ...
                    if (typeof obj != "undefined") {
                        ...
                        if ($scope.config.DashboardType == 2 || window.myglobals.ismobile == true) {
                            if (fPercentage == 0) {
                                status = '<button class="btn btn-mini" type="button">' + $.t(TxtOn) + "</button> " + '<button class="btn btn-mini btn-info" type="button">' + $.t(TxtOff) + "</button>";
                            } else {
                                status = '<button class="btn btn-mini btn-info" type="button">' + $.t(TxtOn) + ": " + fPercentage + "% </button> " + '<button class="btn btn-mini" type="button">' + $.t(TxtOff) + "</button>";
                            }
                            if ($(id + " #status").html() != status) {
                                $(id + " #status").html(status);
                            }
                            ...
                            if (dtype == "relay") imgname = "Fireplace48_O";
                            if (fPercentage == 0) {
                    ...
                    if (dtype != "relay") $.setDimValue = setInterval(function() {
                ...
                stop: function(event, ui) {
                    var idx = $(this).data("idx");
                    var dtype = $(this).slider("option", "type");
                    if (dtype == "relay") SetDimValue(idx, ui.value);
                }
            });
            $scope.ResizeDimSliders();
            $("#dashcontent .selectorlevels div").buttonset({
                create: function(event, ui) {
                    var div$ = $(this), idx = div$.data("idx"), type = div$.data("type"), isprotected = div$.data("isprotected"), disabled = div$.data("disabled"), level = div$.data("level"), levelname = div$.data("levelname");
                    if (disabled === true) {
            ...
            $("#dashcontent .selectorlevels select").selectmenu({
                ...
                value: 0,
                create: function(event, ui) {
                    var select$ = $(this), idx = select$.data("idx"), isprotected = select$.data("isprotected"), disabled = select$.data("disabled"), level = select$.data("level"), levelname = select$.data("levelname");
                ...
                change: function(event, ui) {
                    var select$ = $(this), idx = select$.selectmenu("option", "idx"), level = select$.selectmenu().val(), levelname = select$.find('option[value="' + level + '"]').text(), isprotected = select$.selectmenu("option", "isprotected");
                    SwitchSelectorLevel(idx, unescape(levelname), level, RefreshFavorites, isprotected);
