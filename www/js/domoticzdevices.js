...
Device.popupCancelDelay = function () {
	if (Device.popupDelayTimer != 0) clearTimeout(Device.popupDelayTimer);
	Device.popupDelayDevice = "";
	Device.popupDelayTimer = 0;
}
...
function Alert (item) {
	if (arguments.length != 0) {
		...
		this.NotifyLink = "";
		this.status = this.data  + " (" + item.Desc + ")";
		this.data = "";
...
function Door (item) {
	if (arguments.length != 0) {
		...
		this.onClick = "SwitchLight(" + this.index + ",'" + ((this.status == "Closed") ? "On" : "Off") + "'," + Device.switchFunction + "," + this.protected + ");";
		this.data = '';
	}
}
