...
function Device (item) {
	...
	if (arguments.length != 0) {
		...
		this.isprotected = item.Protected;
		this.name = item.Name;
		this.imagetext = '';
	...
	this.drawIcon = function(parent) {
		...
		if ((this.showStatus == true) && (this.smallStatus.length > 0)) {
			...
			if (Device.useSVGtags == true) {
				var oText = makeSVGmultiline({	transform:'translate('+Device.iconSize/2+','+(Device.iconSize+(Device.elementPadding*1.5)+1)+')', 'text-anchor':'middle', 'font-weight':'bold', 'font-size':'80%' }, 
												$.t(this.smallStatus.replace('Watt','W')), '', Device.iconSize*3, 0, Device.elementPadding*2.5+1);
		...
		if (Device.useSVGtags == true) {
			el = makeSVGnode('image', { id: this.uniquename + "_Icon", 
										...
										onclick: (this.moveable == true) ? '' : "Device.popupCancelDelay(); "+(this.controlable ? '' : '$("body").trigger("pageexit"); ')+this.onClick,
	...
	this.drawDetails = function(parent, display) {
		...
		if (Device.useSVGtags == true) {
			...
			el.appendChild(makeSVGnode('rect', { 'class':'header', x:Device.elementPadding, y:Device.elementPadding, rx:Device.elementPadding, ry:Device.elementPadding, 
			...
			gImageGroup.appendChild(makeSVGnode('image',{id: "image", 'xlink:href': this.image, width:Device.iconSize, height:Device.iconSize, opacity: this.image_opacity, 
														 ...
														 onmouseout:(this.onClick.length != 0) ? "cursordefault()" : '', 'pointer-events':'all'}, '', $.t(this.imagetext)));
			if (this.image2 != '') {
    ...
    this.drawButtons = function(parent) {
		...
		var el;
		var iOffset;
		if ((this.LogLink != "") || (this.TimerLink != "") || (this.TimerLink != "") || (this.NotifyLink != "") || (this.forecastURL.length != 0)) {
				if (Device.useSVGtags == true) {
...
function TemperatureSensor (item) {
	if (arguments.length != 0) {
		...
		this.LogLink = this.onClick = "ShowTempLog('#" + Device.contentTag + "','" + Device.backFunction + "','" + this.index + "','" + this.name + "', '" + this.switchTypeVal + "');";
	}
}
...
function Switch (item) {
	if (arguments.length != 0) {
		...
		this.controlable = true;
                this.onClick = "SwitchLight(" + this.index + ",'" + ((this.status == "Off") ? "On" : "Off") + "'," + Device.switchFunction + "," + this.isprotected + ");";
		...
		this.LogLink = "ShowLightLog(" + this.index + ",'" + this.name + "', '#" + Device.contentTag + "', '" + Device.backFunction + "');";
	...
	if (arguments.length != 0) {
		...
		if (item.Status == 'Closed') {
			this.image = 'images/blinds48sel.png';
                        this.onClick = 'SwitchLight(' + this.index + ",'" + ((item.SwitchType == "Blinds Inverted") ? 'On' : 'Off') + "'," + Device.switchFunction + ',' + this.isprotected +');';
			this.image2 = 'images/blindsopen48.png';
                        this.onClick2 = 'SwitchLight(' + this.index + ",'" + ((item.SwitchType == "Blinds Inverted") ? 'Off' : 'On') + "'," + Device.switchFunction +',' + this.isprotected +');';
		}
		else {
...
function Contact (item) {
	if (arguments.length != 0) {
		...
		this.LogLink = this.onClick = "ShowLightLog('" + this.index + "','" + this.name + "','#" + Device.contentTag + "','" + Device.backFunction + "');";
	}
}
	...
	if (arguments.length != 0) {
		...
		if (item.CustomImage == 0) {
			this.image = (this.status == "Closed") ? "images/" + item.TypeImg + "48.png" : this.image = "images/" + item.TypeImg + "48open.png";
                ...
                this.onClick = "SwitchLight(" + this.index + ",'" + ((this.status == "Closed") ? "On" : "Off") + "'," + Device.switchFunction + "," + this.isprotected + ");";
		this.data = '';
	}
}
...
function Group (item) {
	if (arguments.length != 0) {
		...
		this.image = 'images/push48.png';
                this.onClick = 'SwitchScene(' + this.index + ", 'On', undefined, " + this.isprotected + ');';
		this.image2 = 'images/pushoff48.png';
                this.onClick2 = 'SwitchScene(' + this.index + ", 'Off', undefined, " + this.isprotected + ');';
...
function Humidity (item) {
	if (arguments.length != 0) {
		...
		this.LogLink = this.onClick = "ShowPercentageLog('#" + Device.contentTag + "','" + Device.backFunction + "'," + this.index + ",'" + this.name + "');";
	...
	if (arguments.length != 0) {
		...
		if (item.InternalState=="On") {
			...
			this.image = "images/push48.png";
                ...
                this.onClick = "SwitchLight(" + this.index + ",'On'," + Device.switchFunction + "," + this.isprotected + ");";
	}
}
...
function Pushoff (item) {
	if (arguments.length != 0) {
		...
		this.image = "images/pushoff48.png";
                this.onClick = "SwitchLight(" + this.index + ",'Off'," + Device.switchFunction + "," + this.isprotected + ");";
	}
}
...
function Scene (item) {
	if (arguments.length != 0) {
		item.Status = 'Off';
		this.parent.constructor (item);
                this.onClick = 'SwitchScene(' + this.index + ", 'On','" + Device.backFunction + "', " + this.isprotected + ');';
		this.data = '';
		this.LogLink = '';
...
function Text (item) {
	if (arguments.length != 0) {
		this.parent.constructor (item);
		this.LogLink = this.onClick = this.NotifyLink = "";
		this.status = this.data;
		this.data = "";
	}
}
