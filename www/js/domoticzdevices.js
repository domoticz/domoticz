...
function TemperatureSensor(item) {
	if (arguments.length != 0) {
		...
		this.LogLink = this.onClick = "ShowTempLog('#" + Device.contentTag + "','" + Device.backFunction + "','" + this.index + "','" + this.name + "', '" + this.type + "');";
	}
}
...
function Door(item) {
	if (arguments.length != 0) {
		this.parent.constructor(item);
		if (item.CustomImage == 0) {
			this.image = (this.status == "Locked") ? "images/" + item.TypeImg + "48.png" : this.image = "images/" + item.TypeImg + "48open.png";
		}
