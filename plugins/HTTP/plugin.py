...
<plugin key="Google" name="Goolgle Home page example" author="Dnpwwo" version="1.0.0" externallink="https://www.google.com">
    <params>
        ...
        <param field="Mode1" label="URL" width="500px" default="/"/>
        <param field="Mode2" label="Query String" width="500px" default=""/>
        <param field="Mode3" label="Verb" width="75px">
            <options>
                <option label="POST" value="POST"/>
                <option label="GET" value="GET"  default="true" />
            </options>
        </param>
...
class BasePlugin:
    ...
    def onConnect(self, Status, Description):
        if (Status == 0):
            ...
            headers = { 'Content-Type': 'text/xml; charset=utf-8', \
                        ...
                        'Content-Length' : "%d"%(len(Parameters["Mode2"])) }
            Domoticz.Send(Parameters["Mode2"], Parameters["Mode3"], Parameters["Mode1"], headers)
        else:
    ...
    def onMessage(self, Data, Status, Extra):
        ...
        if (Status == 200):
            ...
            with open(Parameters["HomeFolder"]+"Response.html", "w") as text_file:
