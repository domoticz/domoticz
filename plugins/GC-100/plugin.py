...
#
"""
<plugin key="GC-100" name="Global Cache 100" author="dnpwwo" version="2.1.2" externallink="//http://www.globalcache.com/products/gc-100/models1/">
    <params>
    ...
    def onMessage(self, Connection, Data):
        ...
        try:
            ...
            if (Connection.Name == "Beacon"):
                if (strData.find("<-Model=GC-100") >= 0):
                    if (strData.find("-UUID=GC100_"+Parameters["Address"]+"_GlobalCache") >= 0):
                        self.GC100Conn = None
                        ...
                        Domoticz.Log("GC-100 '"+Parameters["Address"]+"' discovered successfully at address: "+Connection.Address)
                    else:
                        Domoticz.Debug("Saw discovery message for wrong GC-100")
                else:
                    Domoticz.Debug("Saw discovery message for device other than GC-100")
