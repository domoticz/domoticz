...
#
"""
<plugin key="GC-100" name="Global Cache 100" author="dnpwwo" version="1.0.1" externallink="//http://www.globalcache.com/products/gc-100/models1/">
    <params>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        DumpConfigToLog()
        Domoticz.Transport("TCP/IP", Parameters["Address"], str(4998))
    ...
    def onConnect(self, Status, Description):
        ...
        if (Status == 0):
            ...
            Domoticz.Log("Connected successfully to: "+Parameters["Address"]+":4998")
            Domoticz.Send("getdevices\r")
        else:
            self.isConnected = False
            Domoticz.Log("Failed to connect ("+str(Status)+") to: "+Parameters["Address"]+":4998")
            Domoticz.Debug("Failed to connect ("+str(Status)+") to: "+Parameters["Address"]+":4998 with error: "+Description)
    ...
    def onMessage(self, Data, Status, Extra):
        ...
        try:
            ...
            if (action == "state"):
                ...
                Domoticz.Send(Message="getstate,"+str(self.relayDeviceNo)+":1\r")
                Domoticz.Send(Message="getstate,"+str(self.relayDeviceNo)+":2\r", Delay=2)
                Domoticz.Send(Message="getstate,"+str(self.relayDeviceNo)+":3\r", Delay=4)
            else:
    ...
    def onHeartbeat(self):
        ...
        if (self.isConnected == False):
            ...
            Domoticz.Send(Message="getstate,"+str(self.relayDeviceNo)+":"+str(self.lastPolled)+"\r")
