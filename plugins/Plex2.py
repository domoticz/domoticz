...
#
"""
<plugin key="Plex2" name="Plex Home page example" author="corbin" version="1.0.0" externallink="https://www.google.com">
    <params>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        DumpConfigToLog()
        Domoticz.Transport(Transport="TCP/IP", Address="192.168.5.28", Port="11000")
    ...
    def onConnect(self, Status, Description):
        if (Status == 0):
            Domoticz.Debug("Plex connected successfully.")
            data = ''
    ...
    def onMessage(self, Data, Status, Extra):
        ...
        if (Status == 200):
            Domoticz.Log("Good Response received from Plex.")
        elif (Status == 302):
            Domoticz.Log("Plex returned a Page Moved Error.")
            headers = { 'Content-Type': 'text/xml; charset=utf-8', \
                        'Connection': 'keep-alive', \
                        'Accept': 'Content-Type: text/html; charset=UTF-8', \
                        'Host': Parameters["Address"]+":"+Parameters["Port"], \
            ...
            Domoticz.Error("Plex returned a Bad Request Error.")
        elif (Status == 500):
            Domoticz.Error("Plex returned a Server Error.")
        else:
            Domoticz.Error("Plex returned a status: "+str(Status))
