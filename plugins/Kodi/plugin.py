...
#
"""
<plugin key="Kodi" name="Kodi Players" author="dnpwwo" version="1.6.0" wikilink="http://www.domoticz.com/wiki/plugins/Kodi.html" externallink="https://kodi.tv/">
    <params>
...
class BasePlugin:
    ...
    def onNotification(self, Name, Subject, Text, Status, Priority, Sound, ImageFile):
        ...
        if Parameters["Mode3"] == "True":
            import socket
            import xmbcclient3
            packet = xmbcclient3.PacketNOTIFICATION(str(Name), str(Text), xmbcclient3.ICON_PNG, str(ImageFile));
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            if Parameters["Mode6"] == "Debug":
                messbytes = packet.get_udp_message(1)
                outstr = ""
                for x in messbytes:
                    outstr = outstr + hex(x)[2:] + ' '
                Domoticz.Debug(outstr)
