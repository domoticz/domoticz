...
{
	socket_->async_read_some(boost::asio::buffer(buffer_), [this, self = shared_from_this()](auto &&err, auto &&bytes) { handleRead(err, bytes); });
}
	...
	std::size_t bytes_transferred)
{
	...
	if (!e)
	{
			...
			if (recstr.find("AUTH")!=std::string::npos)
			{
				...
				if (strarray.size()==3)
				{
					...
					if (!m_bIsLoggedIn)
					{
						...
						boost::asio::async_write(*socket_, boost::asio::buffer("NOAUTH", 6), [this, self](auto &&err, auto) { handleWrite(err); });
		...
		//ready for next read
		socket_->async_read_some(boost::asio::buffer(buffer_), [this, self](auto &&err, auto bytes) { handleRead(err, bytes); });
	}
...
void CTCPClient::write(const char *pData, size_t Length)
{
	if (!m_bIsLoggedIn)
		return;
	boost::asio::async_write(*socket_, boost::asio::buffer(pData, Length), [this, self = shared_from_this()](auto &&err, auto) { handleWrite(err); });
}
